-- Research/export views for P8 historical performance and trade analysis.

CREATE OR REPLACE VIEW research_account_daily_performance_v1 AS
...
SELECT
    ...
    COALESCE(
        fills.trade_date,
        (COALESCE(fills.matched_at, orders.inserted_at, orders.created_at) AT TIME ZONE 'Asia/Shanghai')::date
    ) AS trade_date,
...
FROM orders
LEFT JOIN fills ON fills.account_id = orders.account_id AND fills.gateway_order_id = orders.gateway_order_id;
