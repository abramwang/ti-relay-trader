ALTER TABLE position_snapshots
    ADD COLUMN IF NOT EXISTS snapshot_type TEXT NOT NULL DEFAULT 'close';
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'position_snapshots_type_check'
    ) THEN
        ALTER TABLE position_snapshots
...
ALTER TABLE position_snapshots
    ...
    UNIQUE (trade_date, account_id, snapshot_type, symbol, exchange);

DROP INDEX IF EXISTS position_snapshots_account_date_idx;

CREATE INDEX IF NOT EXISTS position_snapshots_account_date_type_idx
    ON position_snapshots(account_id, trade_date, snapshot_type);
