...
1. 增加 API Console 可命名保存/导出的请求样例和响应断言集合。
2. 增加券商测试环境批量下单视图和写链路回归；生产只读套件继续严格禁止写请求。
3. 将现有临时 PostgreSQL migration/repository 集成脚本接入外部 CI。
4. 增加盘后对账和绩效贡献的更多固定样例数据测试；人工复核报告已有 API 固定样例和生产只读 Playwright 验收。
