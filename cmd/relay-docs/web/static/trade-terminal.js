(() => {
  ...
  function performanceQualityFlagLabel(value) {
    return {
      ...
      corporate_action_quantity_adjusted: "公司行为数量已校正",
      corporate_action_price_adjustment: "公司行为仅作价格除权",
      corporate_action_mismatch: "公司行为因子与券商持仓不一致",
      corporate_action_check_unavailable: "公司行为因子检查不可用",
      unexplained_open_quantity_change: "日初持仓数量变化无法解释",
      opening_position_without_previous_cost: "日初持仓缺少上一日成本状态",
      etf_t0_cost_separated: "ETF T0 成本已与底仓隔离",
      etf_t0_redemption_cost_released: "ETF 赎回已释放 T0 买入成本",
      etf_t0_cost_bucket_incomplete: "ETF T0 成本池未闭合",
