...
var (
	...
	docs = []docPage{
		...
		},
		{
			...
			Description: "沪深股票、ETF、可转债的权威最小价位、质量状态、Relay 接入与验收结果。",
		},
		{
...
func (s *portalServer) handleJobStatus(w http.ResponseWriter, r *http.Request) {
	...
	s.render(w, pageData{
		...
		Head:       template.HTML(`<link rel="stylesheet" href="/assets/job-status.css?v=20260914-0002">`),
		Content:    template.HTML(body.String()),
...
func (s *portalServer) handleOperationsStatus(w http.ResponseWriter, r *http.Request) {
	...
	s.render(w, pageData{
		...
		Head:       template.HTML(`<link rel="stylesheet" href="/assets/operations-status.css?v=20260921-1443">`),
		Content:    template.HTML(body.String()),
		Scripts:    template.HTML(`<script defer src="/assets/operations-status.js?v=20260921-1443"></script>`),
		ProjectDir: s.root,
	})
}
