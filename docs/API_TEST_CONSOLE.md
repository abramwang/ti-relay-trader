...
现阶段测试台只作为 API 联调页面骨架。9092 文档门户模式不连接 Redis、不写入交易命令；切换到 API 模式并配置数据库、Redis 和账户路由后，`POST /v1/orders` 可发送测试账户下单命令。
...
批量下单、撤单、资金持仓查询、订单成交查询和事件流接口已经在页面中占位，但标记为 `planned`，当前页面不会发送这些未实现请求。
...
4. `POST /v1/orders` 仅在 API 模式下可用，且账户配置必须 `enabled=true`、`trading_enabled=true`。
