...
现阶段测试台作为 API 联调页面骨架。9092 文档门户模式不连接 Redis、不写入交易命令；切换到 API 模式并配置数据库、Redis 和账户路由后，可发送测试账户下单、撤单和账本查询请求。
...
批量下单、资金持仓查询和事件流接口已经在页面中占位，但标记为 `planned`，当前页面不会发送这些未实现请求。
...
4. 交易写接口仅在 API 模式下可用，且账户配置必须 `enabled=true`、`trading_enabled=true`。
