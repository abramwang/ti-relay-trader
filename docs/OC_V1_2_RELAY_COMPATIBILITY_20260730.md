...
次交易日复验已确认 heartbeat 五项 readiness、订单/成交实时与查询身份一致、ETF transfer 分流和撤单拒绝独立审计生效；7 月 30 日的实时订单上下文错位未再复现。

新发现 OC 查询命令 ACK/PEL 恢复链路未收口：六账户 `cmd.query` 合计 18 条 pending，并产生 6 条字段错位的 `BAD_RECOVERED_COMMAND`。详细证据和 OC 修改要求见 `docs/OC_V1_2_VALIDATION_REPORT_20260731.md`。生产交易开关继续保持关闭。
