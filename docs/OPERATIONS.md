...
若盘中当前账本已被后续查询覆盖，不得再使用上述模式把当前值标成日初。此时只允许从 `raw_stream_messages` 按原始 `origin_message_id` 恢复，并逐账户验证唯一成功终态、路由账户、证券集合、数量及捕获时间。资金和持仓证据分别判定：仅持仓证据完整时只恢复 open 持仓，open 资金继续缺失并触发前收盘经济资产回退；只有存在同一盘前窗口的成功资金回报时才能恢复完整 open 资产。原任务失败终态和错误报告必须保留，恢复证据通过 `report.recovery` 与快照 `raw_payload` 追加审计。
