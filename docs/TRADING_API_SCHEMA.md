...
华鑫 TEST 7x24 柜台若在当前 `counter_session_id` 返回精确的全局状态拒单 `BROKER_REJECTED / 当前状态禁止此项操作`，Relay 会在该拒单后 5 分钟内设置 `order_entry_ready=false`、`order_entry_block_reason=TEST_COUNTER_STATE_REJECTED_COOLDOWN` 和 `order_entry_cooldown=true`，并通过 `last_issue_code/message/at` 暴露证据。冷却仅适用于 `environment=test + counter_mode=huaxin_7x24_test`，旧 OC 进程会话的拒单不会阻断新会话，其他业务拒单及生产环境完全不受影响。该保护用于限制已知失败状态下的连续写入，OC heartbeat 的权威 `accepting_trade_commands` 仍是长期修复方向。
