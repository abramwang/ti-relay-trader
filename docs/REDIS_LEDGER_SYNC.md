...
1. `reply`：完整归档到 `raw_stream_messages`，并合并 `asset_page/position_page/order_page/fill_page`；下单类 `rejected/failed` reply 会回写订单终态和错误信息，`BROKER_NOT_READY` 只作为可重试基础设施状态归档。
...
5. `order.cancel.event`：accepted/rejected 均幂等写入 `order_cancel_attempts`，只有 rejected 发布 `order.cancel.rejected` SSE；两者均不修改 `orders`。
...
| 柜台未就绪 | `code=BROKER_NOT_READY` | 归档原始回包并跳过账本状态更新，策略/任务稍后重试 |
