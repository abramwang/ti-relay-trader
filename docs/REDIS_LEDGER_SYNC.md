...
4. `fills` 以 `account_id + gateway_order_id + fill_id` 或 fallback 唯一键去重；Redis Stream 自身仍以 `stream_key + stream_id` 保证重复消费幂等。
...
`tmp/relay_sdk_017_feedback_20260614.md` 中两笔订单表现为订单已 `filled` 但成交明细为空。排查 Redis `event` stream 后确认前置程序已经发送对应 `fill.event`，问题不在 stream 丢消息，而是旧 DDL 将 `fills(account_id, fill_id)` 当作唯一键；测试前置/模拟柜台在不同订单间复用了 `fill_id/match_stream_id`，`ON CONFLICT DO NOTHING` 静默丢弃了后到的合法成交。
