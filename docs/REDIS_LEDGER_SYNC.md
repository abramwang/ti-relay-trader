...
`relayctl ledger-sync` 命令是受控批处理入口，不会写入 `cmd.trade` 或 `cmd.query`，也不会移动 consumer group 位点。自动资金持仓刷新只在 9092 docs/api 常驻服务的轻量后台同步循环中启用。
...
1. 增加 worker 常驻模式，持续消费 `reply/event/hb/dlq`。
2. 引入消费位点表或 consumer group，避免每次从 `0` 回放。
3. 将自动资金持仓刷新迁移到正式 worker，并把调度状态和最近触发时间落盘。
