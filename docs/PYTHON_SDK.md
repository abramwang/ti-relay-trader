...
5. 同时支持实盘账户和券商测试环境账户，使用同一套方法；历史行情模拟撮合由回测引擎负责，不放在 SDK 交易接口中。

## 当前状态

源码包已落在 `sdk/python/relay_sdk`，当前版本号 `0.1.40`。当前实现不依赖第三方 Python 包，使用标准库 HTTP 客户端，便于策略机在内网环境直接 editable 安装或通过 tar.gz 包安装。
...
16. 9092 `/sdk/relay-sdk-0.1.40.tar.gz` 和 `.sha256` 下载入口。
17. `AccountReadiness` 暴露 OC 托管凭据的非敏感状态、版本、Key ID、来源和账户身份；SDK 不提供凭据写入能力。
...
29. 三类写回执稳定返回动作身份：单笔 `order.submit`、批量 `order.batch.submit`、撤单 `order.cancel`；首次发布与相同命令重放保持一致，撤单重放不产生第二次 Redis 副作用。
30. `list_cancel_attempts_page()`、`iter_cancel_attempt_pages()` 和 `iter_cancel_attempts()` 提供持久化撤单尝试证据，断线后无需依赖 SSE 重建；`ORDER_NOT_FOUND` 不改变原订单状态。
...
```bash
python -m pip install "http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.40.tar.gz"
```

校验文件：

```bash
curl -O http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.40.tar.gz
curl -O http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.40.tar.gz.sha256
sha256sum -c relay-sdk-0.1.40.tar.gz.sha256
```
...
`record_settlement_snapshot(snapshot_type="open")` 会写日初资产快照和日初持仓快照；`snapshot_type="close"` 会写收盘资产、日终持仓快照和 reconciliation run。`dry_run=True` 可用于检查输入和返回摘要，不写库。`captured_at` 仅用于故障恢复，必须传入与 `trade_date` 同日、带东八区偏移的 RFC3339 时间；普通流程应留空，由 Relay 使用当前业务时间。`snapshot_only=True` 必须同时提供 `captured_at`，只写源资金/持仓，不读取当前订单成交、不重估行情、不写 reconciliation。
...
| `rebuild_economic_nav_reconciliation(trade_date=..., observed_trade_date=...)` | `POST /v1/accounts/{account_id}/performance/economic-nav/reconcile` | 落库 T+1 净值对账结果；需要服务器开启绩效写入 |
| `confirm_nav_reconciliation(trade_date=..., operator=..., force=False)` | `POST /v1/accounts/{account_id}/performance/nav-reconciliations/confirm` | 人工确认 T+1 对账并将 current economic NAV 推进为 finalized；需要服务器开启绩效写入 |
| `block_nav_reconciliation(trade_date=..., operator=...)` | `POST /v1/accounts/{account_id}/performance/nav-reconciliations/block` | 阻断 T+1 对账并将 current economic NAV 标记为 blocked；需要服务器开启绩效写入 |
...
| `record_settlement_snapshot(...)` | `POST /v1/settlements/snapshots` | 盘前 open 资产/持仓快照、收盘 close 资产/持仓快照和 reconciliation run 落盘 |
