...
| 权威行情复算 | `performance_canonical` | 17:10-19:10 每 10 分钟检查 | Meridian 16:30 启动、16:40/16:50 校准复验、17:05 状态机截止；三类未复权日线水位到达当日后重算 NAV，保留前后版本并审计差异 |
...
`performance_canonical` 读取 Meridian `/v1/quality/postclose-reference`，只有 schema、目标交易日、整体状态以及 `daily_bar_stock_none/daily_bar_etf_none/daily_bar_index_none` 三个 `published_watermark` 全部通过门禁后才执行。Meridian 当前父任务在 16:30 启动，随后于 16:40/16:50 完成本地校准、质量复验和发布，状态机截止为 17:05；Relay 在 17:10 首查，随后每 10 分钟重试至 19:10，保留 125 分钟恢复窗口。同一交易日轮询复用一个任务记录；19:10 仍未通过门禁时转为 Meridian 上游阻塞。门禁未通过时不调用 OC、不覆盖快照也不重算；通过后只重建尚带 Level1 降级标记的已启用账户，再运行同日绩效质量检查。显式账户名单也不能绕过 `enabled=false`，盘后流水线只处理本轮已成功取得 broker close 的账户。canonical 价格已落地但账户因资金连续性等账务质量保持 blocked 时，任务本身记成功并保留账户级告警；只有水位、执行、发布来源或可发布账户失败才判任务失败。`performance_nav_versions` 保留旧 provisional 版本并新增 `meridian_1d_pre_close_and_close` 版本，任务报告保存旧/新主键、版本、NAV、PnL、收益率和差额；差额超过 `50 CNY` 或 `0.1 bp` 中较高门限时告警。成功标记和 PostgreSQL 已完成报告共同保证同日幂等，服务或 cron 重启不会重复生成版本。
...
jobs:
  ...
  performance_canonical:
    enabled: true
    schedule: "10 17 * * 1-5"
    upstream_schedule: "30 16 * * 1-5"
    ready_deadline: "17:05"
    retry_until: "19:10"
```
