...
| 收盘后结算 | `post_close_settlement` | 生产默认 15:05 `Asia/Shanghai` | 追平回报、刷新终态账本、生成日终快照、对账和盈亏输入 |

生产环境默认在交易日 15:05 执行，确保仍处在当前生产 OC/券商柜台查询可用窗口内。测试环境可按联调需要手工触发或调整 cron，但配置和日志都必须明确是 `Asia/Shanghai`。
