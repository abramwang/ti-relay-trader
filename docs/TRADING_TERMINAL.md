...
| `cmd/relay-docs/web/static/echarts.min.js` | 本地 ECharts 运行时，用于绩效分析分钟线和买卖点标注 |
...
8. 底部状态栏：API、Redis 延迟口径、交易阶段和 `Asia/Shanghai` 服务时间。
...
4. `绩效分析`：承接收盘后 close 快照、日终权益/PnL、净值序列、CSV 导出和 Meridian bars 基准数据检查；左侧展示当日分钟线，按当前账户、标的和交易日叠加买卖订单/成交点位；不再单独提供“盘后对账”页面入口。

`交易测试` 视图中的右侧持仓区域采用压缩版资金摘要、工具栏和表格行高，给行情下单与底部委托表留出更多可视空间；`资金持仓` 独立工作区仍保留完整资金拆分和分页持仓表。

`绩效分析` 的分钟线使用同源 `/v1/meridian/market/bars`，默认请求 1m、`09:30:00` 到 `15:00:00`、最多 300 条。若页面输入的是当天或空日期，而当天不是交易日，后端会调用 Meridian 交易日接口取得最近交易日后读取 bars，并把实际交易日回填到页面输入框。

分钟线买卖点来自 relay 本地账本，不新增行情字段定义：
...
5. 打开绩效页、重新查询 bars 或收到 `order.changed/fill.changed` SSE 事件时，会刷新当前图表标的的标注。
...
1. 行情/盘口当前通过 Meridian `/v1/market/snapshots` 获取；如果当日不是交易日，relay 会先调用 Meridian `/v1/metadata/trading-day` 取得最近交易日再读取 historical 快照。绩效页分钟线通过 Meridian `/v1/market/bars` 获取，当 `trade_date` 为空或等于东八区当天时也会按同一交易日口径回退。
