...
4. 历史数据：通过 Meridian `bars` 拉取账表计算所需的日线/分钟线行情；`metadata/instruments` 和 `snapshots` 仅作为交易页面代码补全和行情展示薄代理。
...
7. 实现 Python 盘前初始化和收盘后结算任务。
8. 实现 Python 盘后对账任务。
9. 接入 Meridian 历史数据，补充 PnL 和账户统计。
10. 暂缓内置模拟柜台；实盘调试使用券商测试环境，历史行情模拟撮合放在回测引擎。如未来接入外部模拟柜台，也通过前置服务和 Redis Stream 协议进入 relay。
