...
华鑫 7x24 TEST 柜台有多个交易区间，当前运维方式是在切换区间时由操作员手工重启 OC。
因此采用最小实现：一次 OC 进程生命周期就是一个柜台会话。OC 不需要识别四个区间、交易日历或
结算时刻；Relay 继续负责时段准入和跨会话证据判断。
...
请提供不透明字符串 `counter_session_id`，建议最长 128 字符。该值表示一次 OC 进程会话，不是
日期字段，Relay 不解析其格式。字段应在以下消息中保持一致：
...
1. OC 进程启动时生成一次新 ID，并在首次可用 heartbeat 中报告。
2. 同一进程内的柜台短线断开重连、Redis 重连不得改变 ID。
3. OC 进程重启后生成新 ID；新 ID 必须先出现在 heartbeat，再接受该进程的交易命令。
4. OC 不根据自然日、交易区间或时钟主动轮换 ID，也不需要内置华鑫 7x24 的四段时间表。
5. 同一进程产生的 `order.event`、`order_page` 和撤单结果使用同一个 ID。
6. OC 无法确认升级前历史订单所属会话时应留空，并保留原始柜台上下文；不得填当前会话 ID。
7. 生产正式柜台可以把该字段作为进程会话审计信息，但不得改变生产准入、订单终态或自动处置逻辑。
...
1. 同一 OC 进程内短线断开并恢复后，heartbeat、订单查询/事件和撤单结果的 ID 不变。
2. 人工重启 OC 后 heartbeat 使用新 ID，不要求 OC 判断本次重启对应哪个 7x24 交易区间。
3. 对上一进程会话订单拒撤时，撤单尝试携带当前进程会话 ID；Relay 可证明其与原订单会话不同。
4. 当前会话 ID 缺失或相同，Relay 的人工处置必须失败关闭。
5. 时钟跨过 7x24 区间边界但 OC 进程未重启时，ID 保持不变；OC 不做时段兼容。
