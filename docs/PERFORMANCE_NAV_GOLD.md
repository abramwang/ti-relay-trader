...
- 原先缺正式 NAV 的 `2026-07-31`、`08-03..06`、`08-24..25` 七日均已恢复，不再以 `legacy_cash_snapshot_diagnostic` 出现在该区间。
- `2026-07-27 09:50:10+08:00` 的 `3,100 CNY` 银证转出作为 confirmed `external_flow` 进入收益率分母；逆回购净息、红利税和公司行为现金事实按券商流水独立审计。
- 券商文件虽从 `2026-06-01` 开始，但 Relay/OC 最早账本始于 `2026-06-15` 且缺当日日初持仓；因此 `06-01..07-21` 只作为外部资产金标，不据此伪造证券成本账或前移正式起算日。
...
早期订单/费用核验见 `docs/DEBT5_HISTORICAL_LEDGER_RECONCILIATION_20260804.md`；本次资金与资产基数修复见 `docs/BROKER_CASH_FLOW_AUDIT_20260827.md`。
