...
5. 股票和 ETF 公司行为校正及 ETF T0/底仓分账已升级至 `performance_position_cost.v3.1`；`2026-08-05` 两个真实 `159381.SZ` 申赎账户已完成 PCF、订单组、49 个实物成分划转/卖出和券商持仓联合验收，`2026-08-07/10` 分批公募返款已闭合。当前只剩 `2026-08-06` 两户隔夜资金桥等待券商资金流水核对。
...
- [x] 按可信锚点和数据质量开放首批绩效发布：首批账户先完成 v2.4 连续发布；资金流水审计后，`307000051387/1388` 的 `2026-08-07..26` 共 28 个账户日已升级为 `performance_economic_nav.v2.5 provisional` 且 0 blocked；空账户 `307000051389` 保持 not_applicable。
...
- [ ] 补齐华鑫申赎 T+1/T+2 的多柜台资金口径：当前日线、分钟 IOPV、交易、持仓及真实返款已闭合，但 `2026-08-06` 两户仍有 `-74,811.87 / -95,070.64` 元残差。需要 OC 华鑫适配在能力允许时返回极速/普通柜台资金范围、占款/冻结和按需划转事实；CSV 无余额，不能据此强行发布该日。该限制不得外推到未来接入的其他券商。
...
- `2026-07-31`：历史缺口已于 8 月 17 日恢复。三户当日 15 点后柜台资金、券商交割单、次一交易日日初持仓和 Meridian 未复权日线形成完整审计链；三户 NAV preview 均为 provisional，债享5号日末资产与人工金标精确到分。连续 NAV 待与 8 月 6 日资金桥一起验收后发布。
