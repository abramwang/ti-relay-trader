...
- [x] 建立 OC 新版本后的每日绩效计算任务。交易日 17:45 在 Meridian 盘后归档后，对可信账户先试算移动成本账，再试算经济净值；任务保存逐账户 `ready/attention/blocked`、订单费用覆盖、质量标记和摘要，任一账户失败不把整批其他账户标记失败。任务只读，不绕过绩效写保护。
