...
4. 已取得五个账户的券商交割单并完成外部核验；交割单只用于排错、OC 验收和人工金标，不进入标准交易账本。每日 OC 实时推送和当日权威查询继续作为最终交易事实来源。
...
- [x] 完成 `2026-08-04` OC 兼容修复生产复测：54 条六账户查询全部唯一 completed final，144 个正持仓成本完整；2,190 笔订单、2,128 笔普通成交、49 条 ETF 划转和 2,190 条费用通过 26 项盘中检查，命令组 pending/lag 和今日新增 DLQ 均为 0。
