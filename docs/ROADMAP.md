...
4. 已取得五个账户的券商交割单并完成权威性评估；下一步通过独立、版本化、可 dry-run 的导入和重建流程补齐历史订单经济事实、费用与公司行为，不调用 OC 补查历史。
...
- [x] 完成五账户券商交割单权威性评估：共 109,451 行，覆盖 2026-06-01..2026-08-03；`307000051387/2026-07-30` 的 28 条普通买卖全部唯一匹配已有订单，14 个 OC 错配订单可由交割单安全修复。原始 CSV 含账户流水，只保存在本机并排除 Git；详见 `docs/BROKER_STATEMENT_AUTHORITY_ASSESSMENT_20260804.md`。
- [ ] 实现版本化交割单导入：新增不可变批次/原始行/订单链接/修复审计表，CSV parser 按普通买卖、ETF 申赎、逆回购、现金与公司行为、状态流水分类，先提供 dry-run，不直接修改生产账本。
- [ ] 以 `307000051387/2026-07-30` 执行首个受控修复：审计替换 14 条错误 `relay-summary:*`，落入交割单数量、金额、时间和实际费用，关闭订单成交数量差异和证券数量桥后重算后续成本与净值。
- [ ] 评估其余账户和日期的整日覆盖、唯一订单匹配和起算资产/持仓锚点；没有稳定订单 ID 的旧日期保留为 statement ledger，不生成伪造 OC 订单编号。
