...
7. OC v1.2 的 PEL 恢复、实时账本、ETF 划转、订单实际费用和撤单拒绝账户语义已经通过 `2026-08-03` 完整交易日验收；当前协议数据仍保持闭合，但 `2026-09-21` 生产部署出现一账户三进程的重复启动故障，须先完成实例收敛和启动互斥再恢复正常查询验收。
...
- [ ] 修复生产 OC 重复启动：四户当前各有 1 个健康会话和 2 个未登录重复会话；停止重复实例、收敛 `08:55/09:00` 启动入口并增加账户级进程锁，随后完成唯一会话、四类查询、Stream 和 DLQ 复验。详见 [OC 生产重复会话事故](/home/ti-relay-trader/docs/OC_PROD_DUPLICATE_SESSION_INCIDENT_20260921.md:1)。
