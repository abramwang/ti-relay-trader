...
6. OC v1.2 的 PEL 恢复、实时账本、ETF 划转、订单实际费用和撤单拒绝账户语义已经通过 `2026-08-03` 完整交易日验收；资金查询矛盾终态、Relay 日任务终态门禁和持仓成本增强又于 `2026-08-04` 完成六账户真实回包复测。当前不再有待 OC 修改项，只保留低频异常路径的机会性验收。
7. N12 已完成 API Console 断言集合、批量下单工作台和 API/worker 独立常驻进程；当前继续收敛 N13 绩效数据质量、页面可读性和查询性能。
8. [x] 将盘后 OC 权威查询从行情结算中拆出：15:01 `post_close_capture` 不依赖 Meridian，先固化 `broker_close` 资金/持仓；`post_close_settlement` 再从该不可变输入生成正式 close，行情故障只延后结算和绩效，不再导致券商收盘数据漏采。
