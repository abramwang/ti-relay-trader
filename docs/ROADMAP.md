...
- [ ] 接入实际费用与账户级费用规则；OC/柜台实际费用优先，估算费用标明规则版本，缺少费用时不得将净收益标记为 finalized。生产审计确认订单/成交 `fee/nFee` 和资产 `commission/nCommission` 当前均为 0；债享5号 7 月 23 日 156.72 万元股票卖出对应 1,075.89 元、6.865bp 归因残差，已确认是连续曲线当前阻断项。需 OC 优先核对盘后委托、成交或交割单/资金明细查询，或由用户提供该账户独立费率合同参数。
