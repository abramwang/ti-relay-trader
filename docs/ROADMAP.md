...
7. OC v1.2 的 PEL 恢复、实时账本、ETF 划转、订单实际费用和撤单拒绝账户语义已经通过 `2026-08-03` 完整交易日验收；资金查询矛盾终态、Relay 日任务终态门禁和持仓成本增强又于 `2026-08-04` 完成六账户真实回包复测。当前不再有待 OC 修改项，只保留低频异常路径的机会性验收。
...
- [ ] 在用户协调的维护窗口逐账户替换/重启生产 OC；凭据 heartbeat、查询和重启恢复全部验收后再清理旧明文配置，生产下单仍需单独明确授权。
