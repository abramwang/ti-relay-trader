...
3. 修复证券贡献与主净值没有闭环、零盈亏与缺失盈亏混淆、费用规则缺失和质量状态未阻断正式曲线等问题；四账户验收通过后再扩展历史重建。
4. 完成 OC v1.2 联合验收：下一次启动使用现存 PEL 验证 `QUERY_INTERRUPTED`、pending 清零且不再产生伪 `BAD_RECOVERED_COMMAND`。
5. N12 暂列次优先级：后续补 API Console 断言集合、券商测试环境批量下单视图，以及 API/worker 独立常驻进程。
...
- [x] 将正式经济净值改为 `可见资金 + 行情重估持仓 + 逆回购应收 + 已确认结算调整`，并用资金流加权分母计算日收益。
- [ ] 修复历史资产快照中逆回购本金可能已包含在 `cash_total-cash_available`、同时又加入逆回购应收的重复计入；以 `314000046830/2026-07-10` 为回归样本，并保留冻结资金和普通开放订单资金的可解释拆分。
- [x] 建立移动加权成本账 v1：从可信锚点继承上一日结余，按普通二级市场成交和费用滚动，保存日初/买入/卖出/日终数量、总成本、单位成本和已实现盈亏；公司行为仍待接入。
...
- [ ] 接入实际费用与账户级费用规则；OC/柜台实际费用优先，估算费用标明规则版本，缺少费用时不得将净收益标记为 finalized。生产审计确认订单/成交 `fee/nFee` 和资产 `commission/nCommission` 当前均为 0，需 OC 优先核对盘后委托、成交或交割单/资金明细查询是否能返回结算费用。
- [ ] 公司行为使用 Meridian 除权因子调整日初估值和成本数量关系；原始柜台持仓不被研究侧修正覆盖。
- [ ] 为未来 ETF 申赎预留 `CORE` 与 `ETF_T0:{group_id}` 虚拟成本分账；首批三个新账户在出现首笔申赎前不启用占款、PCF现金替代和跨市场回款估算。
- [x] 绩效页面增加成本连续性、估值来源、公式版本和正式/阻断状态；被阻断日期不计入正式收益曲线，无 v2 NAV 的旧现金快照只作诊断。
- [ ] 回算并验收首批四账户：计算引擎已逐日跑通；`307000051388`、`314000046830` 已生成 provisional v2，`307000051389` 为零资产正确阻断；`307000051387` 的 `2026-07-29` 百万元出金已确认并解除当日阻断，`2026-07-30` 仍受历史成交身份错配阻断。待费用参数和权威成交回填后完成人工金标验收。
- [ ] 四账户通过后，按可信锚点和数据质量分批重建其他账户；原始柜台数据与旧公式版本只读保留，不原地覆盖。
...
- `314000046830`：当前起算锚点保守设在 `2026-07-29`，因此只持久化了 `2026-07-29` 至 `2026-07-30` 两条 provisional v2，并非更早账本不存在。只读逐日试算确认：`2026-06-25` 是最早行情估值和数量输入完整的单日；但 `2026-07-02` 存在已知 OC 历史成交身份异常，不能从 6 月连续发布正式曲线。无需额外修正即可先扩到 `2026-07-24`；若修复 `2026-07-10` 逆回购本金在 `cash_total` 和应收中重复计入、确认 `2026-07-17` 盘中减少的 `3,000,000` 元资金流，并补实际费用，则可信起点可继续提前至 `2026-07-03`。费用需求见 `docs/OC_ACTUAL_FEE_DATA_REQUEST_20260801.md`。
...
- `2026-07-31`：三个有资产账户都缺 close 资产/持仓快照，当日正式绩效继续阻断。

`314000046830` 历史边界补充：当前仅两天是因为起算锚点保守设置为 `2026-07-29`，不是数据库只保存了两天。资金/订单/成交从 `2026-06-15` 开始，Meridian 可估值日线从 `2026-06-22` 开始；但早期存在 OC 成交身份错配，且 `2026-07-29` 以前没有 open 持仓快照。`2026-07-09` 日终证券持仓为零，因此 `2026-07-10` 日初的 `5,614,624.70` 元可见资金是当前最早不依赖污染历史成本的干净重建锚点，可把连续试算扩展到 `2026-07-10..2026-07-30` 共 15 个交易日。正式迁移前需补实际费用，并确认 `2026-07-27` 相对前一日减少的 `10,000` 元隔夜资金变化；`512760.SH` 二级市场交易还表明策略范围应包含 ETF 截面，而不应只标为股票截面。
...
- 四账户连续交易日回算结果通过人工金标准核对后，才替换绩效首页主曲线口径。
