...
- [ ] 修正 v2 逆回购口径：本金只在未进入可见资金时加入，预估利息仅作诊断，取得实际回款/费用后再进入正式收益。以 `314000046830/2026-07-02` 和 `2026-07-10` 为本金嵌入样本、`2026-07-14` 为独立应收样本，显式拆分逆回购占款、冻结资金、卖出待取资金和普通开放订单资金，字段不足时阻断而非按日期特判。
- [x] 将用户确认的债享5号 2026 年 7 月 17 个交易日资产/盈利表保存为人工金标，并完成本金单计、预估利息和隔夜调整的逐日对账；详见 `docs/DEBT5_MANUAL_NAV_RECONCILIATION_20260801.md`。
...
- `314000046830`：当前起算锚点保守设在 `2026-07-29`，因此只持久化了 `2026-07-29` 至 `2026-07-30` 两条 provisional v2，并非更早账本不存在。用户已提供 `2026-07-01..31` 的 17 条人工资产/盈利金标：7 月 22、23、28、29、30 日在本金只计一次且不计预估利息后与 Relay 精确到分一致，7 月 14 日只差 0.72 元；7 月 2 日和 10 日资金快照已包含逆回购本金，7 月 17 日可见资金减少的 300 万由连续人工资产证明属于不可见占款/柜台资金，不是外部出金。实现本金重叠拆分、实际费用和人工金标导入后，可重建 7 月账户级曲线；7 月 2 日的历史 OC 成交身份异常仍独立阻断证券级贡献，7 月 31 日缺 OC close 快照也不能被人工值伪装为原始观测。费用需求见 `docs/OC_ACTUAL_FEE_DATA_REQUEST_20260801.md`。
