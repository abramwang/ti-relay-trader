...
3. 使用 OC `fee.list.query` 的订单级权威费用继续修复证券贡献、成本账与主净值闭环；当前日费用自动落库，历史费用通过交割单或审计导入补齐后再扩展历史重建。
...
- [ ] 补齐历史实际费用。OC 的 `broker_order_fund_detail` 当前只支持柜台当前交易日，不能回查 7 月历史；债享5号 7 月 23 日 1,075.89 元、6.865bp 残差仍需交割单/资金明细导入或账户费率合同参数，历史缺费日期不得标记 finalized。
