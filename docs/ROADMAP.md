...
- [x] 完成五账户券商交割单外部核验：共 109,451 行，覆盖 2026-06-01..2026-08-03；`307000051387/2026-07-30` 的 28 条普通买卖全部唯一匹配已有订单，14 个 OC 错配订单得到外部证实。交割单只用于排错、OC 验收和人工金标，不写入标准账本；详见 `docs/BROKER_STATEMENT_AUTHORITY_ASSESSMENT_20260804.md`。
- [x] 明确历史边界：OC 只负责交易日当天数据，Relay 不建设日常手工交割单导入，不生成伪造 OC 订单/成交身份；历史异常保留证据并阻断正式绩效。
- [x] 完成 56 条历史 DLQ 一次性修复和审核：14 条错误 Relay summary 按 14/14 唯一交割单匹配修正，6 个订单成交量差异 resolved；32 条 `FILL_ORDER_CONTEXT_MISMATCH` 和 18 条恢复查询为 acknowledged，6 条旧解析错误为 ignored，pending=0。原始 Redis/DLQ 永久保留，详见 `docs/DLQ_ONE_TIME_REPAIR_20260804.md`。
