...
5. 股票和 ETF 公司行为校正及 ETF T0/底仓分账已升级至 `performance_position_cost.v3.1`；`2026-08-05` 两个真实 `159381.SZ` 申赎账户已完成 PCF、订单组、49 个实物成分划转/卖出和券商持仓联合验收，`2026-08-07/10` 分批公募返款已闭合；`2026-08-06` 隔夜资金桥也已由开户以来券商资产基数和 ETF 待结算承接证据闭合。
...
- [ ] 为 ETF T0/底仓混合账户实现 `meridian_pre_close_mark_to_market` 绩效起算成本源；由用户确认富盈13号起算日后，使用盘前数量和 Meridian 前收盘价建立 CORE 初始成本，不信任被申赎污染的柜台成本。
