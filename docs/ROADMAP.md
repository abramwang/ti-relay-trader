...
- [ ] 接入账户级费用规则；实际费用优先，估算费用标明规则版本，缺少费用时不得将净收益标记为 finalized。
...
- [ ] 回算并验收首批四账户：计算引擎已逐日跑通；`307000051388`、`314000046830` 已生成 provisional v2，`307000051389` 为零资产正确阻断，`307000051387` 受 `2026-07-29` 疑似外部出金和 `2026-07-30` 历史成交身份错配阻断；待费用参数、外部资金流确认及权威成交回填后完成人工金标验收。
...
- `307000051387`：`2026-07-29` 净值与证券贡献残差为 `-1,000,458.48`，与未入账的 `-1,000,000` 外部资金流高度一致，需人工确认后才能记账。`2026-07-30` 有 18 个证券数量差异；例如 `510810.SH` 委托成交 `76,400`，成交表仅有 `300`，属历史 OC 成交身份错配，必须使用权威查询/重放修复，Relay 不生成合成成交。
