...
3. 下一个交易日使用 OC `fee.list.query` 获取六账户订单实际费用，按账户、市场、证券类型、方向和业务类型校准版本化费率；用户确认开户后费率未调整，可将校准规则向历史有效区间外推，但历史结果必须标记 `estimated_from_broker_calibration`，不得伪装成历史柜台原始费用。
4. 为缺 OC close 快照的交易日建立衍生日终口径：优先用下一交易日 open 快照反推，其次才使用当日 open、完整订单成交、资金流水和 Meridian close 正向推演；衍生结果独立保存并经 T+1 残差门禁，只能 provisional，不覆盖原始 OC 快照。
5. 使用 Meridian `adjust-factors` 完成股票和 ETF 公司行为校正，再继续首批四账户连续区间重建。
...
- [ ] 建立“最新实费校准、历史估算回填”链路。按账户、市场、证券类型、买卖方向和业务类型从最新 `order_fee_records` 分项推导佣金率、最低佣金、印花税、过户费及其他费率，生成带样本范围、有效期、确认人和版本的 fee rule；用户确认这些账户开户后费率未调整，因此规则可回溯至账户起算日。覆盖不足的维度继续标记缺失，历史费用来源统一为 `estimated_from_broker_calibration`，并与真实 `broker_order_fund_detail` 分账保存。
- [ ] 为缺 OC close 快照的日期生成可审计衍生日终观察。优先使用下一交易日 open 的实际资金/持仓，扣除隔夜外部流、收益和公司行为后反推；下一 open 尚不可用时，使用当日 open 数量、完整普通成交/ETF 划转、确认资金流水和 Meridian close 正向估值。只有数量桥闭合且现金残差不超过阈值时才生成 provisional，来源、输入和残差完整保存，不写回 `asset_snapshots/position_snapshots` 冒充 OC 数据。
- [ ] 公司行为接入 Meridian `/v1/metadata/adjust-factors`：股票和 ETF 统一按 `ex_date/ex_factor/ex_cum_factor` 校正跨日价格和单位成本；物理数量以券商 open 持仓为权威，因子只用于预期数量桥与成本连续性校验。现金分红、送转和 ETF 份额折算分别标记，红利现金进入资金桥时不得与除权收益重复计算。
...
- `2026-07-31`：三个有资产账户都缺 OC close 资产/持仓快照，当前正式绩效继续阻断；下一个交易日取得 open 快照后，可按衍生日终口径反推 7 月 31 日并进行 T+1 残差验收。
