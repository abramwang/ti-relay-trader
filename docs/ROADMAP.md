...
7. OC v1.2 的 PEL 恢复、实时账本、ETF 划转、订单实际费用和撤单拒绝账户语义已经通过 `2026-08-03` 完整交易日验收；当前协议数据仍保持闭合。`2026-09-21` 重复会话根因已确认是测试机遗留生产 OC crontab，须停止残留进程、删除定时任务并完成唯一会话复验。
...
- [ ] 清理测试机遗留生产 OC crontab 和四个仍活跃的未登录残留进程；生产机每户保留唯一健康会话，随后完成唯一会话、四类查询、Stream 和 DLQ 复验。启动入口仍应增加 `env + broker + account_id` 进程锁，详见 [OC 生产重复会话事故](/home/ti-relay-trader/docs/OC_PROD_DUPLICATE_SESSION_INCIDENT_20260921.md:1)。
