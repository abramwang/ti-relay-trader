...
- [ ] 在下一交易日盘前用实际资金、持仓和公募回款做 T+1 对账；到账事实通过确认结算流水或权威清算输入覆盖估值，不把 IOPV 估值升级为 finalized 清算事实。
...
- [x] 将 `performance_daily` 从独立 17:45 cron 改为 `post_close_settlement` 成功依赖：15:01 盘后报告成功持久化且 close 快照无错误后立即使用同一交易日计算；结算失败或非交易日不触发。配置/API/任务页暴露 `job_success` 和 `depends_on`，cron 由仓库脚本统一安装。
