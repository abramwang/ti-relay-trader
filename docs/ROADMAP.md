...
- [x] 以 `2026-07-24` 的订单、成交、ETF 成分划转和 Meridian PCF 梳理策略事实：两个账户存在完整 ETF 申赎 T0 链路，一个账户同时存在 ETF 截面调仓；当日未观察到独立股票截面调仓。
- [ ] 与用户确认正式净值、日内资产桥、逆回购和 ETF 申赎归因口径；确认前不修改计算逻辑或回填历史数据。
