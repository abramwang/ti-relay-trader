...
7. OC v1.2 的 PEL 恢复、实时账本、ETF 划转、订单实际费用和撤单拒绝账户语义已经通过 `2026-08-03` 完整交易日验收；当前协议数据仍保持闭合。`2026-09-21` 重复会话已定位到公网出口 `117.186.20.114` 的五个旧版 OC，具体主机或守护入口待排查。
...
- [ ] 定位并清理公网出口 `117.186.20.114` 的五个旧版 OC 进程及守护入口；保留 `210.22.105.118` 的四个健康生产会话，随后完成唯一会话、四类查询、Stream 和 DLQ 复验。启动入口仍应增加 `env + broker + account_id` 进程锁，详见 [OC 生产重复会话事故](/home/ti-relay-trader/docs/OC_PROD_DUPLICATE_SESSION_INCIDENT_20260921.md:1)。
