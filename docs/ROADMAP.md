...
- [ ] 后续按交易日用交割单抽查 OC 当日订单、成交、费用和现金影响，但最终标准账本始终使用 OC 当日数据。
...
- [x] 用 `2026-08-07` 实际资金桥完成两户公募返款对账：两户均确认到账 `73,448.94` 元，来源于 `2026-08-05` ETF T0。`performance_economic_nav.v2.3` 将来源日待结算估值纳入到账日日初经济资产并冲销，只把实际到账与原估值之差列为 ETF 清算差额；返款不记外部入金，字段不完整时阻断。
- [x] 用 `2026-08-10` 资金桥识别同一 `2026-08-05` 赎回的第二批公募返款：涌盈波动率到账 `13,214.60` 元，智算汇利混合到账 `53,214.50` 元。`performance_economic_nav.v2.4` 增加分期清算审计链，补充到账显式设置 `released_estimate=0`，不重复释放已于 8 月 7 日冲销的来源估值。
