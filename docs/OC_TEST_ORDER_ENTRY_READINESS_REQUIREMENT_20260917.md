...
If the broker API exposes no proactive state field, OC should at minimum lower `accepting_trade_commands` after receiving this exact global state rejection and keep it false until a reliable recovery signal or process restart. Relay independently applies a five-minute TEST-only circuit as a bounded fallback.
