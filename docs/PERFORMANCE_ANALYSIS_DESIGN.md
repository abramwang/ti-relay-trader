...
3. 正式数据质量区固定检查六类输入：资产快照与资金桥、Meridian 基准 bars、证券贡献、订单成交账本、经济净值与 T+1 NAV 对账、盘前初始化与盘后结算。
...
7. `tests/integration/performance_visual_smoke.py` 使用生产只读数据验证区间查询、canvas 有效像素、六项质量检查和无浏览器/HTTP 错误；`1600x1280` 与 `1280x800` 均无横向溢出。
