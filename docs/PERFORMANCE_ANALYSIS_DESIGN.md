...
OC `broker_order_fund_detail`、订单、成交和资金持仓查询均以当前柜台能力为边界，Relay 不把最新交易日费用外推成历史柜台费用，也不生成衍生 close 冒充 OC 原始快照。7 月历史继续只读保留现有诊断和人工金标；后续取得券商交割单时，另行设计带来源文件哈希、导入批次、版本、冲突检查和人工确认的历史导入流程，再受控重建历史成本与净值。
