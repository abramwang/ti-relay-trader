...
9. `performance_position_cost.v3` 接入 Meridian `adjust-factors`。券商 open 数量与上一 close 数量之比匹配 `ex_factor` 时，保持持仓总成本并调整单位成本；因子存在但数量不变时按价格除权留痕；因子与券商数量无法闭合时标记 `corporate_action_mismatch` 并阻断。原始因子上下文独立保存，不覆盖 OC 持仓。
...
4. `CORE` 与 `ETF_T0:{group_id}` 独立成本池及 PCF 最小申赎单位门禁已完成；后续取得数据后补现金替代、跨市场代买代卖和公募回款的版本化结算输入。
