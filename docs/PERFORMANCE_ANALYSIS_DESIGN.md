...
4. `performance_economic_nav.v2.4` 以 `可见资金 + Meridian 持仓重估 + 未进入可见资金的逆回购本金 + ETF待结算资产 + 确认调整` 计算日初/日终 NAV。日初持仓使用 Meridian `pre_close`，日终使用 `close`；已确认公募返款按每笔 `released_estimate` 释放来源日待结算估值，再把实际到账与本笔释放估值之差单列为 ETF 清算差额。补充分期返款可显式使用 `released_estimate=0`，避免重复释放已经由首笔到账冲销的估值。预估逆回购利息只作诊断，柜台 `avg_cost/market_value/unrealized_pnl` 仅用于输入对账。
...
1. 首个 OC 新版本完整交易日已完成每日绩效试算；截至 `2026-08-17`，三个活跃账户的 `2026-08-07..17` 七个完整交易日已按 `performance_economic_nav.v2.4` 连续发布为 provisional，21 组成本/NAV 均非 blocked。可信空起点且资金、持仓、订单、成交和资金流水均为零的账户保持 `not_applicable`，不生成收益率和严重告警。后续盘后流水线只由本机 `relayctl` 发布非阻断结果，公网绩效设置写开关仍关闭。
