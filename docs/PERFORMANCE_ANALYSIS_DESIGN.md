...
2. 历史费用、旧成交缺口和缺失 OC close 快照不推断回填；券商交割单只用于离线核验和人工金标，不导入标准交易账本，历史异常继续阻断正式绩效。
...
OC `broker_order_fund_detail`、订单、成交和资金持仓查询均以当前柜台能力为边界，Relay 不把最新交易日费用外推成历史柜台费用，也不生成衍生 close 冒充 OC 原始快照。7 月历史继续只读保留现有诊断和人工金标；券商交割单只作为一次性外部核验证据，不建设导入批次、自动重建或标准化手工补数流程。每日 OC 当日数据是正式交易账本的唯一来源。
