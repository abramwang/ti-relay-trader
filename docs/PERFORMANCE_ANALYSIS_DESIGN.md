...
2. OC 暂不提供完整资金流水时，Relay 只允许人工维护已明确确认的外部入金/出金和单次审计调整；柜台间划转、红利、利息、费用、逆回购回款和清算事实应优先由 OC 当日接口进入同一标准账表，不能按日期或策略自动推断。
...
IOPV+15bp 只回答交易日内的估算盈亏。T+1/T+2 或更晚取得公募真实现金差额、现金替代和完整费用后，Relay 使用版本化 `performance_etf_settlement_versions` 把来源交易日升级为最终清算口径：

```text
final_gross_pnl = actual_component_sale_gross
                + actual_cash_component
                + actual_cash_substitution
                - actual_etf_buy_gross
final_net_pnl   = final_gross_pnl - actual_total_fee
```

最终清算必须同时满足：

1. 赎回数量是目标日 Meridian `etf_cash_component.v1.unit_subscribe_redeem` 的整数倍，PCF 日期与 Schema 版本可审计。
2. ETF 买入量/成交额、OC 实际 transfer 对应的成分卖出量/成交额与来源日交易账本闭合；跨市场未划入成分不得由 PCF 虚构成交。
3. `actual_cash_component` 和 `actual_cash_substitution` 来自真实券商资金证据。PCF 预计 cash component 只能用于对账，不能在到账前冒充终值。
4. 实际费用覆盖 ETF 买入、赎回父订单和关联成分卖出；确认终值后这些订单视为费用已覆盖，不再叠加 15bp。
5. 来源日 IOPV 待结算 carry 在后续到账日按带符号金额释放。公募扣款形成负 carry 时也必须保留，不能因金额小于零而丢弃。
6. 同一来源日/ETF 的修订生成新版本并退役旧 current 版本；原始 OC、Meridian 和券商证据均不覆盖。

添利1号 `501000114077` 的首个完整样本已完成：`2026-07-22..2026-08-24` 共 14 个 `159915.SZ` 赎回来源日由暂估升级为终值，24 个连续 NAV 日的策略归因残差均为 0。`2026-08-25` 的 `512700.SH` 赎回虽然已有下一交易日 PCF 预计现金差额，但券商文件截至 8 月 26 日尚无对应真实到账，因此保持 pending，8 月 25/26 日不发布伪终值。完整证据见 `docs/TIANLI1_ETF_SETTLEMENT_RECONCILIATION_20260828.md`。
...
| 确认资金流水 | 扩展后的 `cash_ledger` | OC 当日资金事实、明确外部入出金和一次性审计调整 |
...
| 异常订单 | 拒单缺少原因、非拒单残留柜台错误、数量不一致或状态冲突的订单；有完整原因的业务拒单不属于数据质量异常 |
...
5. 使用已确认资金流水把 `overnight_adjustment` 分解为 `external_flow`、`internal_transfer`、`settlement_adjustment`、`interest_dividend`、`fee_tax_adjustment` 等；日常优先使用 OC 当日事实，人工仅处理明确外部资金或一次性事故审计。
...
`2026-08-01` 生产审计确认，所有实盘账户的订单和成交 `fee` 均为 0；“债享5号”原始 Redis 归档中的 `fee/nFee` 出现 56,207 次但全部为 `0.0`，99 条资产快照的 `commission` 也全部为 0。Relay 已完成字段接收和落库，因此这不是字段映射遗漏。OC 通用结构已预留 `TiRtnOrderStatus.nFee`、`TiRtnOrderMatch.nFee` 和 `TiRspAccountInfo.nCommission`，下一步应先核对华鑫盘后委托/成交查询是否会补齐实际费用；若费用仅存在交割单或资金明细，OC 应增加独立只读查询并保留佣金、印花税、过户费、经手费等分项及结算状态。

实际费用接入规则：成交级费用最优；只有订单总费用时按订单只计一次，证券归因可按各成交金额分摊但必须标记 `allocated_from_order_fee`；只有账户日汇总费用时只用于账户净收益和费用总额核对，不伪装成精确证券费用。盘中预估费用与盘后结算费用分别保存，盘后值通过版本更新替代计算来源，禁止重复扣减。

交易数据完整性与费用完整性是两道独立门禁。订单身份、交易日、证券、方向、数量、价格、状态以及订单成交关联完整时，即使费用缺失，该订单仍可用于数量桥、移动成本、成交额和毛收益计算，不得标记为无效订单。费用门禁只覆盖当日有成交的唯一订单；费用未齐时，净收益使用明确标记的估算值或保持费用待定，结果为 `provisional` 并标记 `order_fee_day_incomplete/broker_delivery_statement_pending`，待券商交割单补齐后才能 finalized。

## 人工外部资金与柜台间划转

当前 OC 华鑫适配没有完整资金流水和多柜台余额接口，只能取得华鑫极速柜台资金；这是华鑫柜台能力边界，不是 OC 对未来券商的统一限制。用户可在绩效设置中人工维护明确发生的外部入出金；极速/普通柜台间划转、公募占款和 ETF 清算应优先由对应券商的 OC 当日事件或查询提供。券商导出文件只用于调查已知事故的一次性审计，不建设定时、批量或常规导入流程。现有 `cash_ledger` 作为统一账本，不另建平行口径。
...
第一版页面在 `/trade#performance-settings` 提供“费用规则”和“资金流水”两个 tab。资金流水支持账户、日期、分类和状态过滤，提供明确外部资金的新增、确认、冲正和导出；不提供券商流水常规导入。该页面是生产敏感写入口，不复用交易权限开关，需要独立的绩效配置写权限和完整审计。柜台间划转只有在 OC 事件或人工明确确认两条腿时才入账，不能按申赎日期自动生成。
...
1. 首屏当日 KPI 固定展示日初经济净值、日终经济净值、当日盈亏和当日收益率；下一层展示外部资金、成交额、OC 实际费用和成本数量桥，再显示公式版本、Meridian 估值来源、成本起点和费用覆盖。区间累计收益、回撤、基准和超额收益收敛到图表区，不再与当日指标混排。
...
4. ETF 成分股卖出只有在 OC 实际 `transfer.event` 与卖出成交按赎回篮子、证券、时间和数量闭合后，才从 IOPV 估值收益及普通持仓成本中排除。PCF 不用于生成 transfer；跨市场代买代卖、现金替代或其它未实际划入账户的成分不会被 Relay 虚构为持仓/成交。
5. `/trade#performance` 默认展示证券贡献表，可切换到原净值序列；估算、缺失和排除状态可见。
6. Go 单元测试覆盖普通持仓现金流恒等式、ETF T0 多成交聚合、PCF 最小申赎单位不匹配、实际 transfer 数量关联、未划转跨市场证券不误关联，以及待结算估值公式。
...
3. 异常明细覆盖拒单缺少原因、未终态、订单与成交数量不一致、终态冲突、非拒单柜台错误字段残留和成交缺委托；有完整柜台原因的业务拒单保留在拒单数/拒单率中，不作为数据质量异常。
...
1. 首个 OC 新版本完整交易日已完成每日绩效试算；`307000051387/1388` 已按 `performance_economic_nav.v2.7` 从共同起算日 `2026-07-27` 连续重建到 `2026-08-26`，各 23 日、共 46 个账户日全部 provisional 且无 blocked。可信空起点且资金、持仓、订单、成交和资金流水均为零的账户保持 `not_applicable`，不生成收益率和严重告警。后续盘后流水线只由本机 `relayctl` 发布非阻断结果，公网绩效设置写开关仍关闭。
...
4. `CORE` 与 `ETF_T0:{group_id}` 独立成本池、PCF 最小申赎单位门禁、实际 transfer/成分卖出关联和 IOPV 待结算估值已完成；`000026_etf_settlement_finalizations` 已补齐版本化最终清算输入，并用添利1号 14 个真实清算来源日验收估值消退。后续继续等待 pending 清算的真实公募资金，不提前使用 PCF 预计值确认。
5. 债享5号人工金标已进入独立版本化审计表。历史订单和费用修复后，`2026-07-22..30` 已从证券空仓干净起点连续重建，5/5 可计算金标通过 0.01 元门禁；7 月 31 日仍因缺 OC close 快照 unavailable。下一步继续完成其余三账户金标验收。
6. 给研究侧导出 view 增加 v2 版本，保留 v1 作原始柜台参考。

### 当前历史边界

OC `broker_order_fund_detail`、订单、成交和资金持仓查询均以当前柜台能力为边界，Relay 不把最新交易日费用外推成历史柜台费用，也不生成衍生 close 冒充 OC 原始快照。债享5号已有 OC 交易日经交割单逐笔验收后完成一次性实际费用和旧上下文修复，但不建设导入批次、自动重建或标准化手工补数流程。每日 OC 当日数据仍是正式交易账本的最终来源。

当前每日验收从 OC 新版本后的完整交易日开始。完整性门禁至少包括：当日 open/close 快照存在、订单均终态或有明确未终态说明、订单与成交数量闭合、`fee_page` 汇总与订单费用闭合、Meridian `1d` 或当日 Level1 估值字段及必要公司行为数据可用。门禁按账户独立执行，完整账户正常计算，异常账户保留 blocked 原因。
...
5. 移动加权成本 v3 已用于可信账户并支持公司行为和 ETF T0 分账；历史推断组有歧义、PCF 缺失或结算输入不足时仍不输出精确净收益。
