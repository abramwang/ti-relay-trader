...
当前每日验收从 OC 新版本后的完整交易日开始。完整性门禁至少包括：当日 open/close 快照存在、订单均终态或有明确未终态说明、订单与成交数量闭合、`fee_page` 汇总与订单费用闭合、Meridian 日线和必要公司行为数据可用。门禁按账户独立执行，完整账户正常计算，异常账户保留 blocked 原因。
