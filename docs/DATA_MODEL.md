...
| 表 | 说明 |
| --- | --- |
| `accounts` | 账户配置、所属券商 `broker_id`、账户状态、是否可交易、账户环境类型；券商标签与账户别名分离 |
...
| `performance_position_cost_states` | 逐账户、交易日、证券和成本池保存移动加权成本状态，包含上一 close、券商 open、日内买卖、日终数量与成本、已实现盈亏、Meridian 估值、公司行为因子/类型/原始上下文、柜台数量残差和质量标记 |
| `performance_nav_gold_versions` | 版本化人工净值金标，独立保存承接日初、实际日初、隔夜调整、日末资产、每日盈利、确认审计和原始输入；只用于验收，不参与 NAV 公式 |
| `performance_etf_settlement_versions` | ETF 申赎 T0 的版本化最终清算输入；保存来源交易日、证券、申赎单位、实际买入/成分卖出/现金差额/现金替代/费用、来源日待结算 carry、Meridian PCF 版本和确认审计 |
...
经济净值主线不覆盖原始柜台快照：`asset_snapshots` 继续保存柜台可见资金和持仓市值，`performance_nav_versions` 保存绩效侧的经济净资产版本。`performance_economic_nav.v2.5` 使用柜台可见资金、收盘后已确认清算现金、Meridian `pre_close/close` 重估持仓、未进入可见资金的逆回购本金、ETF 申赎待结算资产和确认调整构建 NAV，不使用柜台 `avg_cost/market_value/unrealized_pnl` 作为研究侧成本与估值。ETF 待结算估值来自已闭合的 OC transfer/成分卖出事实与 IOPV + 15bp 口径；IOPV 优先取历史 Level1，缺失时只允许使用赎回前最后一个完整 1 分钟 bar，并显式记录 fallback。OC 实际费用不完整时只加回已确认金额，保留 pending 质量标记，结果只能 provisional。公募实际返款作为 confirmed `cash_ledger` 结算记录保存，必须带来源交易日、原估值和精确发生时间；发生在不可变 close 快照之后的到账通过 `post_close_settlement_cash` 加入经济 NAV，不覆盖原始快照。首笔到账按 `released_estimate` 冲销日初待结算资产，后续分期通过 `receipt_stage=supplemental`、`settlement_group_id` 和 `prior_receipt_entry_id` 关联前序到账，已无待释放估值时使用 `released_estimate=0`，整笔作为追加清算差额，不作为外部资金流。极速/普通柜台间划转仅在存在 OC 事件或人工明确证据时记账，不因申赎日期自动推断；券商导出只作一次性审计，不进入日常导入链路。逆回购响应和 NAV `pnl_components` 保存本金 `embedded/separate/ambiguous` 判定、现金重叠、正式本金应收、预估净息和两条候选残差；预估利息不进入正式 NAV/PnL，实际到账通过确认的资金流水入账。`performance_position_cost.v3.1` 从已确认起算锚点按移动加权法滚动，并以券商 open 数量为物理事实、Meridian `adjust-factors` 为公司行为校验。普通持仓保存在 `CORE`，按显式 `t0_order_group_id` 或历史闭合规则识别的 ETF 买入/赎回保存在 `ETF_T0:{group_id}`；T0 买入成本不进入底仓，赎回后该日内成本池归零，只有实际 transfer 关联的成分卖出会从 CORE 成本中剥离。`economic-nav/rebuild` 写入时会把同账户同交易日旧 `is_current=true` 版本退役，再插入新的 current 版本；`performance_nav_reconciliations` 按 `performance_nav_pk` 幂等保存 same-day provisional 对账和 T+1 observed open assets 对账结果。人工确认接口会在同一 current NAV 版本上写入 `finalized_at/status=finalized`，阻断接口写入 `status=blocked`。`performance_nav_gold_versions` 与以上计算链路隔离：每次人工修订生成新版本，相同内容按 SHA-256 幂等复用 current，只在公式完成后对比，不作为估值或收益输入。费率、外部资金流、现金替代和跨市场回款继续以版本化输入完善；缺输入时只能输出 provisional 或 blocked，不允许 finalized。生产写入类 HTTP 接口默认关闭，批处理通过受控的部署机 CLI 执行。

`performance_position_cost.v3.2` 在 v3.1 基础上增加 `meridian_pre_close_mark_to_market` 起算成本源。它只在人工确认的起算日使用券商盘前持仓数量乘 Meridian 当日未复权 `pre_close` 建立 CORE 初始成本，适用于 ETF 申赎已经污染柜台平均成本的混合账户；任一正持仓缺少前收盘价时阻断，绝不回退柜台成本。后续交易日仍承接上一日成本并按成交、费用和公司行为滚动，该能力不会自动选择账户或起算日。
...
```text
gross_contribution = component_sale_gross_amount
...
其中 `actual_cash_component` 允许为负，表达公募实际扣款；现金替代和费用不得为负。应用前还必须与 OC 的赎回数量、买入成交额和成分卖出成交额逐项闭合。来源日的 `source_close_settlement_carry` 通过 `cash_ledger.settlement_kind=etf_redemption_final_settlement` 在后续交易日释放，释放额允许带符号，防止负现金差额被错误当作无效流水。若券商权威资产基数已经包含收盘快照后的清算到账，则只打 `post_close_settlement_included_in_broker_asset_basis` 标记，不再把同笔现金重复加到 close NAV。
