...
BIN="${RELAY_DOCS_BIN:-$ROOT_DIR/.runtime/bin/relay-api}"
PREVIOUS_BIN="${RELAY_DOCS_PREVIOUS_BIN:-$ROOT_DIR/.runtime/bin/relay-api.previous}"
PID_FILE="${RELAY_DOCS_PID_FILE:-$ROOT_DIR/.runtime/run/relay-api.pid}"
LOG_FILE="${RELAY_DOCS_LOG_FILE:-/var/log/relay/relay-api.log}"
...
ensure_dirs() {
  mkdir -p "$(dirname "$BIN")" "$(dirname "$PREVIOUS_BIN")" "$(dirname "$PID_FILE")" "$(dirname "$LOG_FILE")" "$(dirname "$CRON_LOG_FILE")"
}
...
build_binary() {
  ...
  if [[ "${RELAY_SKIP_BUILD:-false}" != "true" ]] && needs_build; then
    ...
    (cd "$ROOT_DIR" && go build -o "$candidate" ./cmd/relay-docs)
...
case "${1:-status}" in
  ...
  logs)
    ...
    ;;
  *)
    echo "usage: $0 {start|stop|restart|status|rollback|logs|install-cron|uninstall-cron}" >&2
    exit 2
    ;;
esac
