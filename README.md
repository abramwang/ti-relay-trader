...
| 安全状态 | 测试账户 `00030484` 启用查询和交易；生产配置未修改，生产账户交易权限仍全部关闭 |
| 当前阶段 | P0-P4 完成，P5-P8/P10 持续生产化；N8-N12 完成；N13 可信成本账与绩效重建进行中 |
| 最近确认 | `2026-09-15 15:18 Asia/Shanghai` 按用户明确指令切到测试；API、Redis、事件流和订单服务均为 `ok`，测试 OC `order_entry_ready=true` |
| 更新时间 | `2026-09-15` |
...
6. 当前处于测试环境，账户 `00030484` 可用于策略联调；任何切回生产或调整生产交易权限都必须收到用户新的明确指令。
