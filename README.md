...
- 当前状态: 已推进 P1/P2/P4/P5 部分底座，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台骨架、PostgreSQL 首版账本 migration、真实库 migration 验证、账本 repository 骨架和 Redis reply/event 到账本同步批处理；尚未实现交易核心逻辑。
- 最近更新时间: `2026-06-13`
...
1. 让前置 `order.event.payload` 补齐 `trade_side` 和 `business_type`，或在 relay 下单 API 中先写订单草稿。
2. 将下单 API 接入订单草稿写入和 Redis `cmd.trade` 写入。
3. 设计模拟柜台账表 schema。
...
- 当前观察到的 `order.event.payload` 缺少 `trade_side` 和 `business_type`，所以事件暂只能归档 raw，不能单独重建订单主表；需要前置补字段，或 relay 下单时先写订单草稿。
