...
- 当前状态: 已推进 P1/P2/P4/P5/P6/P7 底座并进入 P8，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台表单化 UI、接口测试台模板/静态资源拆分、成熟交易软件风格 `/trade` 手动测试终端与工作区切换、文档门户同源 `/v1/*` API 挂载、PostgreSQL 账本 migration、真实库 migration 验证、`stream_checkpoints` 位点表、`job_runs` 任务运行表、账本 repository 骨架、Redis reply/event 到账本同步批处理、9092 轻量后台 Redis reply/event 同步循环、正式 worker 模式持久化位点同步、单笔下单 API、批量下单 API、撤单 API、下单幂等重放/冲突保护、资金/持仓查询与前置刷新 API、默认当日订单/成交查询、历史订单/成交/持仓查询口径、账本 API 时间字段东八区格式化和零值时间省略、`POST /v1/settlements/snapshots` 日终快照落盘、`reconciliation_inputs/reconciliation_breaks` 对账输入和差异表、`GET /v1/accounts/{account_id}/performance/daily` 日终权益/PnL 输入汇总、`GET /v1/accounts/{account_id}/performance/series` close 净值绩效序列、`GET /v1/accounts/{account_id}/performance/series.csv` 绩效 CSV 导出、`order_page/fill_page` reply 合并、订单/成交事件驱动的资金持仓自动限频刷新、`GET /v1/events/stream` SSE 实时事件流、`GET /v1/status` 依赖健康/交易阶段/日流程任务状态检查、Python SDK 标准库 HTTP 客户端、订单状态/成交回调和内网 tar.gz 安装包下载入口，以及 Meridian 主数据/快照/bars 薄代理、`/trade` 代码补全/行情刷新、股票 2 位/ETF 3 位价格精度和 `/trade#performance` 绩效分析工作区；已固化 `Asia/Shanghai` 东八区业务时区和交易日盘前初始化/收盘后结算流程，并新增 `python -m relay.jobs.pre_open_init`、`python -m relay.jobs.post_close_settlement` 任务骨架、`--persist` 报告落盘和收盘后 close 快照/reconciliation run 落盘；worker 已能持续消费 `reply/event/hb/dlq` 并恢复 stream 位点，心跳状态建模和 DLQ 告警仍待补充。
