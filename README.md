...
- 当前状态: 已推进 P1/P2/P4/P5/P6/P7 底座并完成 P8，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台表单化 UI、接口测试台模板/静态资源拆分、成熟交易软件风格 `/trade` 手动测试终端与工作区切换、文档门户同源 `/v1/*` API 挂载、PostgreSQL 账本 migration、真实库 migration 验证、`stream_checkpoints` 位点表、`job_runs` 任务运行表、账本 repository 骨架、Redis reply/event 到账本同步批处理、9092 轻量后台 Redis reply/event 同步循环、正式 worker 模式持久化位点同步、单笔下单 API、批量下单 API、撤单 API、下单幂等重放/冲突保护、资金/持仓查询与前置刷新 API、默认当日订单/成交查询、历史订单/成交/持仓查询口径、订单/成交/持仓 cursor 分页、前置/柜台拒单错误信息回写订单账本和 `/trade` 展示、订单累计成交量到成交账本的汇总补齐、成交 `fill_id` 订单作用域去重修复、账本 API 时间字段东八区格式化和零值时间省略、`POST /v1/settlements/snapshots` 日终快照落盘、`reconciliation_inputs/reconciliation_breaks` 对账输入和差异表、`GET /v1/accounts/{account_id}/performance/daily` 日终权益/PnL 输入汇总、`GET /v1/accounts/{account_id}/performance/series` close 净值绩效序列和 Meridian bars 基准对照、`GET /v1/accounts/{account_id}/performance/series.csv` 绩效/基准 CSV 导出、研究侧 PostgreSQL 导出 view、`order_page/fill_page` reply 合并、订单/成交事件驱动的资金持仓自动限频刷新、`GET /v1/events/stream` SSE 实时事件流、`GET /v1/status` 依赖健康/交易阶段/日流程任务状态检查、Python SDK 标准库 HTTP 客户端、订单状态/成交回调、P8 helper 方法和内网 tar.gz 安装包下载入口，以及 Meridian 主数据/快照/bars 薄代理、`/trade` 代码补全/行情刷新、股票 2 位/ETF 3 位价格精度、`/trade#performance` 绩效分析工作区、取消 `/trade` 单独盘后对账入口、订单监控/资金持仓按交易日查询及服务端分页、交易测试视图右侧持仓版面压缩、`/trade` ECharts 分钟 K 线、非交易日 bars 最近交易日回退和买卖订单/成交点位标注；已固化 `Asia/Shanghai` 东八区业务时区和交易日盘前初始化/收盘后结算流程，并新增 `python -m relay.jobs.pre_open_init`、`python -m relay.jobs.post_close_settlement` 任务骨架、`--persist` 报告落盘和收盘后 close 快照/reconciliation run 落盘；下一步进入 P9 模拟柜台账表 schema，worker 心跳状态建模和 DLQ 告警仍待补充。
- 当前 9092 运行态: 使用未跟踪本地配置 `config/relay.local.yaml` 启动文档门户和同源 API，已接入测试 PostgreSQL、测试 Redis 和测试账户 `00030484`；`/v1/status` 可查看数据库、Redis、订单服务、行情代理、事件流和自动刷新状态。`000006_research_performance_views` 已应用到测试库。该文件包含凭据且不提交。
- 最近更新时间: `2026-06-14`
...
1. 进入 P9，设计模拟柜台账户、资金、持仓、订单、成交账表 schema。
...
- 每日交易主流程已完成 Python 任务骨架、任务运行报告落盘、收盘后 close 资产/持仓快照落盘、`reconciliation_runs` 批次 upsert、`reconciliation_inputs` 输入摘要、`reconciliation_breaks` 差异记录和账户日终权益/PnL 输入汇总第一版；下一步需要输出更完整的人工复核报告，并基于 Meridian bars 形成连续绩效序列。
...
- `GET /v1/accounts/{account_id}/performance/series` 当前以 close 净资产为主线计算累计收益和回撤；基准行情、持仓按 bars 复权估值和研究侧导出仍是后续任务。
- `GET /v1/accounts/{account_id}/performance/series.csv` 是轻量 CSV 导出；后续仍需要 PostgreSQL 视图或 Parquet/CSV 批量导出任务，服务研究侧大批量回测数据消费。
- P8 账表计算只接入 Meridian `bars`。交易端暂不接入实时 level2 数据，也不规划 `trades/orders/order-queues`，避免把不存在或非必要的数据源纳入核心路径。
- `GET /v1/meridian/market/bars` 当前是同源薄代理，不做字段映射和持久化；当 `trade_date` 为空或等于东八区当天时，会先调用 Meridian 交易日接口取得 `previous_or_current_trading_date`，非交易日自动回退到最近交易日。后续账表计算会基于该入口读取 `market_bar.v1` 数据并写入绩效序列或研究导出视图。
...
- 2026-06-14 压测暴露测试前置/模拟柜台会在不同订单间复用 `fill_id/match_stream_id`；relay 已通过 `000005_fill_id_order_scope` 把成交唯一键调整为 `account_id + gateway_order_id + fill_id`。Redis Stream 协议仍要求同一订单内成交编号稳定，并建议前置 `fill.event` 始终携带 `gateway_order_id`、`order_id` 和 `order_stream_id`。
- 同轮排查过程中曾观察到原始 `adapter_context.order_status_name=unAccept` 与标准订单 `filled` 不一致；回放完整事件后目标订单 raw 状态已更新为 `dealt`。若后续再次出现 raw status 与标准状态冲突，Relay 继续以标准字段和数量口径保证账本终态，并建议前置/模拟柜台侧澄清原始状态字段语义。
...
- `2026-06-14`: 收尾 P8：`DailyPerformance` 和 CSV 增加 `realized_pnl/gross_pnl/net_pnl` 第一版研究侧口径；新增并应用 `000006_research_performance_views`，提供 `research_account_daily_performance_v1` 与 `research_order_fill_export_v1` 两个 PostgreSQL 导出 view。测试库验证 view 可读：日绩效 view 1 行，订单成交导出 view 199 行。路线图中 P8 已切换为 done，下一步进入 P9 模拟柜台账表 schema。
