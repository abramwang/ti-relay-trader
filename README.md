...
| 安全状态 | 生产新版 OC 已加载 4 户凭据 `v2`；公网出口 `117.186.20.114` 仍运行 5 个旧版 OC 并连接生产 Redis，与 `210.22.105.118` 的正常生产 OC 并存；生产下单关闭 |
| 当前阶段 | P0-P4 完成，P5-P8/P10 持续生产化；N8-N12 完成；N13 可信成本账与绩效重建进行中 |
| 最近确认 | `2026-09-21 10:45 Asia/Shanghai` Redis 连接与 heartbeat 已把异常源定位到 `117.186.20.114`；该源还在运行已停用账户 `307000051389`，具体内网主机仍待按公网出口排查 |
| 更新时间 | `2026-09-21` |
...
- `2026-09-21 10:45 Asia/Shanghai` 通过生产 Redis `CLIENT LIST` 和短时 `MONITOR` 将重复 OC 精确区分：`210.22.105.118` 为四户新版健康生产 OC；`117.186.20.114` 仍有五个旧版 OC，包含四户异常会话和已停用账户 `307000051389`。后者没有托管凭据字段，持续发布新 heartbeat 并可能抢占查询，因此测试机删除 crontab 后仍需按公网出口定位实际残留主机或守护进程。健康会话的当日订单/成交逐户闭合、语义重复为 0；Relay 未重启、未改账，生产交易继续关闭。完整证据见 [OC 生产重复会话事故](/home/ti-relay-trader/docs/OC_PROD_DUPLICATE_SESSION_INCIDENT_20260921.md:1)。
...
1. 先定位公网出口 `117.186.20.114` 对应的主机或 NAT 网络并停止其中五个旧版 OC，保留 `210.22.105.118` 的四个健康生产会话；Relay 随后复验唯一会话、四类只读查询、Stream lag 和 DLQ。未经用户明确指令不得开放生产下单。
