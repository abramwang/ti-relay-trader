...
- `2026-08-05`: 接入 OC 当日订单查询终态修复协议。Relay 继续要求查询具备唯一 `order_page/completed/is_last=true` 终态，并保持真正 partial 后 `QUERY_FAILED` 的账户级阻断；订单页新增保留 item 级 `status_message`、`cancel_reason` 和 `adapter_context`，使 `20046` 等单笔撤单业务原因随对应 cancelled 订单落库和展示，不再丢失或误作查询级错误。兼容依据见 `docs/RELAY_COMPATIBILITY_NOTICE_20260805.md`。API/worker 已生产发布，24 条 output stream 健康、DLQ pending 为 0；因 OC 已于 15:30 下线，下一交易窗口还需对 `314000046830` 连续执行两次只读订单查询，验收唯一 completed final、20046 订单原因和 close 快照恢复。
