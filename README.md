...
- 当前 9092 运行态: 使用未跟踪本地配置 `config/relay.prod.yaml` 启动生产观察模式，`service.environment=production`，生产 Redis ping 正常，账户路由为空，`auto_refresh=false`，不开放下单或查询刷新命令。只读扫描发现生产 stream 前缀 `relay:prod:v1:huaxin:501000114077`，当前存在 `cmd.trade`、`cmd.query` 和 `hb`，其中命令 stream 为空、心跳持续更新。该文件包含凭据且不提交；生产 Redis 凭据只允许进入未跟踪本地配置或安全运行环境，不写入仓库。
- 最近更新时间: `2026-06-15`
...
- 2026-06-15 当前生产观察配置为了更严格只读，暂不配置任何 `accounts`，因此 `/v1/accounts` 返回空，`POST /v1/orders` 和账户刷新接口会被 `account route not found` 拦截；后续若要联调生产账户查询，先把 `501000114077` 作为账户路由加入且保持 `trading_enabled=false`，再评估是否允许 `cmd.query`。
