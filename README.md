...
| 最近确认 | `2026-09-06` 已通过标准脚本切到测试环境；9092、测试 Redis/数据库、事件流、订单服务和 SSE 均健康，页面冒烟 29/29；测试库当前尚无账户资金快照 |
| 更新时间 | `2026-09-06` |
...
- `2026-09-06` 当前运行态已持久切换到测试环境：`.runtime/active-config.yaml -> config/relay.local.yaml`，账户 `00030484` 的查询和交易路由开启；API 使用内嵌账本同步，因此独立 `relay-worker` 按配置停用。测试库尚无该账户资金快照，完整 SDK 账本冒烟会在资产查询返回 404；待测试 OC 在线并完成首次资金查询后再进行 Chronos P1 真实写验收，不伪造快照。
...
6. 当前已切到测试环境；待测试 OC 在线并取得资金快照后，执行 Chronos P1 的批量部分拒绝、`BROKER_NOT_READY` 和结果未知等真实写验收。切回生产前仍保持生产账户只读，不为验收向生产账户制造订单。
