...
- 当前状态: 已推进 P1/P2/P4/P5/P6 部分底座，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台表单化 UI、接口测试台模板/静态资源拆分、成熟交易软件风格 `/trade` 手动测试终端与工作区切换、文档门户同源 `/v1/*` API 挂载、PostgreSQL 首版账本 migration、真实库 migration 验证、账本 repository 骨架、Redis reply/event 到账本同步批处理、9092 轻量后台 Redis reply/event 同步循环、单笔下单 API、批量下单 API、撤单 API、资金/持仓查询与前置刷新 API、订单/成交账本查询与前置刷新 API、`order_page/fill_page` reply 合并、订单/成交事件驱动的资金持仓自动限频刷新、`GET /v1/events/stream` SSE 实时事件流、`GET /v1/status` 依赖健康检查、Python SDK 标准库 HTTP 客户端、订单状态/成交回调和内网 tar.gz 安装包下载入口，以及 Meridian 主数据/快照薄代理、`/trade` 代码补全/行情刷新和股票 2 位/ETF 3 位价格精度；正式位点持久化 worker 尚未实现。
- 当前 9092 运行态: 使用未跟踪本地配置 `config/relay.local.yaml` 启动文档门户和同源 API，已接入测试 PostgreSQL、测试 Redis 和测试账户 `00030484`；`/v1/status` 可查看数据库、Redis、订单服务、行情代理、事件流和自动刷新状态。该文件包含凭据且不提交。
- 最近更新时间: `2026-06-14`
...
| `http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.1.tar.gz` | Python SDK 安装包 |
| `http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.1.tar.gz.sha256` | Python SDK 安装包 SHA256 |
...
1. 增加正式常驻 worker，持久化 stream 位点并持续同步 `reply/event/hb/dlq`。
2. 设计模拟柜台账表 schema。
3. 增加 Python 盘后对账与账户盈亏统计任务骨架。
4. 增加 SDK 集成测试和版本发布检查清单。
5. 增加 9092 页面冒烟测试和 SDK 发布前检查脚本。
...
- Python SDK 当前可用 `PYTHONPATH=sdk/python`、`python -m pip install -e sdk/python` 或 `python -m pip install "http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.1.tar.gz"` 安装；安装包由 `scripts/build-python-sdk.py` 生成并提交到 `public/sdk/`。
