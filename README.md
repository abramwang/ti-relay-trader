...
- `2026-08-01`: 排查“债享5号”净值区间过短：当前 v2 只有 `2026-07-29..30`，原因是起算锚点保守设在 7 月 29 日，不是源数据只有两天。账户账本始于 6 月 15 日、Meridian 可估值日线始于 6 月 22 日，但早期存在 OC 成交身份错配且缺 open 持仓快照；7 月 9 日日终已空仓，故 7 月 10 日日初 `5,614,624.70` 元现金可作为最早不继承污染成本的干净锚点，理论可重建至 7 月 30 日共 15 个交易日。迁移前仍需确认 7 月 27 日隔夜减少的 1 万元资金性质，并把 `512760.SH` 二级市场交易纳入 ETF 截面范围。
- `2026-08-01`: 审计实盘实际费用链路：所有实盘账户订单/成交 `fee` 均为 0，资产快照 `commission` 也均为 0；债享5号原始流中的 `fee/nFee` 56,207 次全部为 `0.0`，证明 Relay 没有漏映射。OC 通用结构已包含订单 `nFee`、成交 `nFee` 和账户 `nCommission`，后续优先要求 OC 核对盘后查询或增加交割单/资金明细查询；订单总费用可按成交金额分摊用于证券归因，账户日汇总费用只作账户净收益和总额核对，均不得与成交费用重复扣减。
