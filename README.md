...
- 当前状态: P0-P4 已完成，P5-P8/P10 持续生产化，N9-N12 已完成。当前主线为 N13 可信成本账与绩效重建。债享5号已从 `2026-07-22` 空仓锚点完成连续重建；富盈13号 `2026-06-18..2026-08-03` 的 16,781 个普通成交订单已与券商交割单全部闭合。`2026-08-05` 两个 `159381.SZ` ETF T0 真实样本的 PCF、49 个实物成分划转和 IOPV + 15bp 待结算估值已经闭合。盘后流程已拆成 `post_close_capture -> post_close_settlement -> performance_daily`：前者在 15:01 仅依赖 OC/Redis/DB，固化最终资金持仓为 `broker_close`；后两步再从不可变快照生成 close、对账和绩效。`2026-08-14` Meridian 异常期间仍成功保存 6 份 `broker_close` 资金和 278 条持仓；数据恢复后已用未复权 `1d` bars 重建正式 close，历史重算不再错误复用当日 Level1 条件，缺收盘估值时会拒绝覆盖已有快照。修正后 278 条正持仓均有有效市值，绩效任务为 3 ready、1 not_applicable、0 attention/blocked。`2026-08-17` 已于 15:09 重新查询六账户最终资金和持仓，覆盖 6 份 `broker_close/close` 资产与 282 条持仓；正式结算和可信账户绩效复算完成，六账户复核均 passed、开放对账断点为 0。`2026-08-06` 缺失券商收盘快照的历史边界保持不补造，详见 `docs/SETTLEMENT_HOLD_20260806.md`。生产保持 6 个只读账户、下单账户 0。
