...
- 当前状态: 已推进 P1/P2/P4/P5/P6/P7 部分底座，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台表单化 UI、接口测试台模板/静态资源拆分、成熟交易软件风格 `/trade` 手动测试终端与工作区切换、文档门户同源 `/v1/*` API 挂载、PostgreSQL 账本 migration、真实库 migration 验证、`stream_checkpoints` 位点表、`job_runs` 任务运行表、账本 repository 骨架、Redis reply/event 到账本同步批处理、9092 轻量后台 Redis reply/event 同步循环、正式 worker 模式持久化位点同步、单笔下单 API、批量下单 API、撤单 API、下单幂等重放/冲突保护、资金/持仓查询与前置刷新 API、默认当日订单/成交查询、历史订单/成交/持仓查询口径、`order_page/fill_page` reply 合并、订单/成交事件驱动的资金持仓自动限频刷新、`GET /v1/events/stream` SSE 实时事件流、`GET /v1/status` 依赖健康/交易阶段/日流程任务状态检查、Python SDK 标准库 HTTP 客户端、订单状态/成交回调和内网 tar.gz 安装包下载入口，以及 Meridian 主数据/快照薄代理、`/trade` 代码补全/行情刷新和股票 2 位/ETF 3 位价格精度；已固化 `Asia/Shanghai` 东八区业务时区和交易日盘前初始化/收盘后结算流程，并新增 `python -m relay.jobs.pre_open_init`、`python -m relay.jobs.post_close_settlement` 任务骨架与 `--persist` 报告落盘；worker 已能持续消费 `reply/event/hb/dlq` 并恢复 stream 位点，心跳状态建模和 DLQ 告警仍待补充。
- 当前 9092 运行态: 使用未跟踪本地配置 `config/relay.local.yaml` 启动文档门户和同源 API，已接入测试 PostgreSQL、测试 Redis 和测试账户 `00030484`；`/v1/status` 可查看数据库、Redis、订单服务、行情代理、事件流和自动刷新状态。`000002_stream_checkpoints` 已应用到测试库。该文件包含凭据且不提交。
- 最近更新时间: `2026-06-14`
...
| `http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.4.tar.gz` | Python SDK 安装包 |
| `http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.4.tar.gz.sha256` | Python SDK 安装包 SHA256 |
...
1. 检查订单/成交/资金/持仓账本 API 的历史时间字段展示是否全部转换为 `Asia/Shanghai`，并清理 `0001-01-01T00:00:00Z` 这类零值展示。
...
- 每日交易主流程已完成 Python 任务骨架和任务运行报告落盘；当前 `post_close_settlement` 还未实际固化日终持仓快照和对账批次。
...
- Python SDK 当前可用 `PYTHONPATH=sdk/python`、`python -m pip install -e sdk/python` 或 `python -m pip install "http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.4.tar.gz"` 安装；安装包由 `scripts/build-python-sdk.py` 生成并提交到 `public/sdk/`。
- 历史持仓查询读取 `position_snapshots`；当前测试库尚未由收盘任务写入日终持仓快照，所以 `/v1/accounts/{account_id}/positions/history` 可能返回空，下一步应在收盘结算中固化快照。
