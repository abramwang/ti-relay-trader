...
- 当前状态: 已推进 P1/P2/P4/P5/P6 部分底座，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台骨架、PostgreSQL 首版账本 migration、真实库 migration 验证、账本 repository 骨架、Redis reply/event 到账本同步批处理，以及单笔下单 API 的订单草稿落盘 + Redis `cmd.trade` 写入；批量下单、撤单、查询和常驻 worker 尚未实现。
- 最近更新时间: `2026-06-13`
...
1. 实现撤单 API：订单状态校验、Redis `order.cancel` 写入和命令归档。
2. 实现订单/成交查询 API，从 PostgreSQL 账本读取。
3. 设计模拟柜台账表 schema。
