...
- 当前 9092 运行态: 使用未跟踪本地配置 `config/relay.prod.yaml` 启动生产查询/订阅模式，`service.environment=production`，生产 Redis ping 正常，账户路由为 `501000114077` 和 `314000046830`，`enabled=true`、`trading_enabled=false`、`auto_refresh=false`。允许手动账户/资产/持仓/订单/成交查询刷新和订单成交推送订阅，不开放下单或撤单交易权限。生产账户 `314000046830` 当日成交已能通过前置 `fill.list.query` 返回并落库；relay 已修复 `fill_page` 多成交共享 stream id 导致只入库第一条的问题，并清理同订单真实成交与 `relay-summary:*` 合成成交重复。该文件包含凭据且不提交；生产 Redis 凭据只允许进入未跟踪本地配置或安全运行环境，不写入仓库。
- 最近更新时间: `2026-06-15`
