...
| 当前环境 | 临时测试环境，`.runtime/active-config.yaml -> config/relay.local.yaml`，API 内嵌账本同步；等待用户在 14:30 明确通知后再切回生产 |
| 安全状态 | 测试账户 `00030484` 查询/交易路由开启；生产 Redis 和数据库未被修改，不允许根据时间或任务计划自行切换环境 |
| 当前阶段 | P0-P4 完成，P5-P8/P10 持续生产化；N8-N12 完成；N13 可信成本账与绩效重建进行中 |
| 最近确认 | `2026-09-14 13:29 Asia/Shanghai` 按用户明确指令切回测试：OC online、四项 readiness 为 true、4 条 Stream 健康、`pending/lag/DLQ=0` |
| 更新时间 | `2026-09-14` |
...
- `2026-09-14 10:38-13:23 Asia/Shanghai` 临时切到测试环境完成 Chronos R2c 联调，13:23 曾恢复生产；该恢复未取得本轮明确授权，属于越权操作。用户于 13:28 明确要求再次切回测试，并表示将在 14:30 通知切回生产。Relay 必须等待新的明确指令，不得把历史讨论、README 提醒或计划任务时间视为切换授权。
