...
1. 补充 Python SDK 打包发布脚本和 `/sdk/relay-sdk-<version>.tar.gz` 下载入口。
2. 设计模拟柜台账表 schema。
...
- Python SDK 当前为源码包骨架，可用 `PYTHONPATH=sdk/python` 或 `python -m pip install -e sdk/python` 进行本地使用；内网 tar.gz 安装包和 9092 `/sdk/` 下载入口仍待发布。
