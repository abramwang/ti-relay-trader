...
1. 将数据库状态接入 `/v1/status`。
2. 增加正式常驻 worker，持久化 stream 位点并持续同步 `reply/event/hb/dlq`。
3. 设计模拟柜台账表 schema。
4. 增加 Python 盘后对账与账户盈亏统计任务骨架。
5. 增加 SDK 集成测试和版本发布检查清单。
