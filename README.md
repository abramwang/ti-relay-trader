...
| 最近确认 | `2026-09-15 22:39 Asia/Shanghai` 按用户明确指令切回生产；API/worker、Redis、PostgreSQL、事件流和订单服务均为 `ok`，`trading_enabled=0` |
| 更新时间 | `2026-09-15` |
...
6. 当前处于生产环境，五个启用账户只读、全部下单关闭；任何切换到测试或开放生产交易都必须收到用户新的明确指令。
