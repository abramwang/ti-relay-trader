...
| 安全状态 | TEST/PROD 托管凭据主密钥仅部署于本机 `0600` 忽略文件；TEST `00030484` 已创建并校验凭据 `v1`，等待 OC 明确 `user_product_info` 来源后重启验收；生产运行态未切换 |
| 当前阶段 | P0-P4 完成，P5-P8/P10 持续生产化；N8-N12 完成；N13 可信成本账与绩效重建进行中 |
| 最近确认 | `2026-09-18 13:29 Asia/Shanghai` 当前仍为测试环境；TEST `00030484` 凭据 `v1` 写入、认证解密与审计通过，未重启 OC、未切换生产 |
| 更新时间 | `2026-09-18` |
...
- `2026-09-18 13:29 Asia/Shanghai` TEST 账户 `00030484` 已按 `oc.secret.v1` 写入首个凭据版本：Redis `current=v1`、AES-GCM 认证解密、信封身份及 PostgreSQL `started/succeeded` 审计全部通过；临时明文文件已删除，仓库与 API 日志泄漏扫描为 0。用户另提供了 `user_product_info`，但 OC 现行协议的明文 JSON 只定义登录用户、登录密码和动态密码，因此该字段未擅自写入信封，也未重启 OC；需先由 OC 明确它来自非敏感公共配置还是扩展协议。
...
1. TEST/PROD 两套 Key ID/Key 已完成本机安全部署，TEST `00030484` 凭据 `v1` 已写入；先让 OC 明确 `user_product_info` 的配置来源，再协调新版 OC 以 `--env test --account-id 00030484` 启动，并验收登录、凭据心跳、查询、最小订单、重启恢复和无明文泄漏，同时把 TEST 从旧 `relay:prod:*` 迁移到 `relay:test:*`。
