...
- `2026-08-26`: 排查富盈13号 `314000045768` 盘后账户级阻断。15:01 的资金、29 条持仓和 201 笔订单查询均取得唯一成功终态，但 `fill.list.query` 被 OC 以 `QUERY_FAILED / 20046:市价单不能满足成交条件撤单` 结束，Relay 因缺少权威成交查询终态拒绝写入该账户 `broker_close`；其余五账户结算通过。15:04 在 OC 在线时补查再次立即返回相同失败，确认不是超时或偶发串包。业务错误实际属于 `300750` 卖单 `external-huaxin-31400004576801-12001A180003653`，订单已为 `cancelled`、成交 `1,600/2,600`，不应污染成交查询。该现象与 `RELAY_COMPATIBILITY_NOTICE_20260805` 已修复的订单查询终止帧污染同源，需要 OC 将相同业务状态隔离规则覆盖到 `fill.list.query`；修复后补查成交并重跑当日完整盘后链路，Relay 不绕过查询终态门禁。详细交接和验收条件见 `docs/OC_FILL_QUERY_TERMINAL_INCIDENT_20260826.md`。
