...
- 当前状态: P0-P4 已完成，P5-P8/P10 持续生产化，N9-N12 已完成。当前主线为 N13 可信成本账与绩效重建。债享5号已从 `2026-07-22` 空仓锚点完成连续重建；富盈13号 `2026-06-18..2026-08-03` 的 16,781 个普通成交订单已与券商交割单全部闭合。`2026-08-05` 两个 `159381.SZ` ETF T0 真实样本的 PCF、49 个实物成分划转和 IOPV + 15bp 待结算估值已经闭合。盘后流程已拆成 `post_close_capture -> post_close_settlement -> performance publish -> performance_daily`：15:01 先固化最终资金持仓为 `broker_close`，生成 close 后由服务器本机 `relayctl` 发布非阻断成本账和 provisional NAV，最后记录质量及 `published/preview_only`；公网绩效写开关保持关闭。`2026-08-14` Meridian 异常恢复后已用未复权 `1d` bars 重建正式 close。`2026-08-17` 已于 15:09 重新查询六账户最终资金和持仓，覆盖 6 份 `broker_close/close` 资产与 282 条持仓；六账户复核均 passed、开放对账断点为 0。智算汇利混合、涌盈波动率和债享5号的 `2026-08-07..17` 七个完整交易日已连续发布 `v2.4` NAV，21 组结果 0 blocked。历史恢复已用可审计来源补齐三户 `2026-07-31` 和 `2026-08-06` 的 `broker_close/close`：持仓使用下一交易日日初并由交割单或逐证券交易桥验证，估值使用 Meridian 未复权 `1d` bars；`2026-07-31` 三户和 `2026-08-06` 债享5号绩效试算可用，两只 ETF T0 账户的 8 月 6 日仍因隔夜公募资金桥未确认保持 blocked，未发布伪精确 NAV。生产保持 6 个只读账户、下单账户 0。
