...
| 安全状态 | 生产仍运行旧版 OC，5 个启用账户只读、0 个开放交易；TEST 托管凭据 `v1` 已验收，生产托管凭据尚未迁移 |
| 当前阶段 | P0-P4 完成，P5-P8/P10 持续生产化；N8-N12 完成；N13 可信成本账与绩效重建进行中 |
| 最近确认 | `2026-09-18 14:02 Asia/Shanghai` 按用户明确指令切回生产；旧版 OC 兼容正常，5 个网关在线、0 个开放交易、总 lag=0 |
| 更新时间 | `2026-09-18` |
...
1. 当前生产保持旧版 OC 和 Relay 只读；等待用户协调维护窗口逐账户替换新版 OC。切换前先录入对应生产账户凭据，切换后验收凭据 heartbeat、查询和重启恢复；未经用户明确指令不得开放生产下单。
