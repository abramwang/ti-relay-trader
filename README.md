...
- 当前状态: P0-P4 已完成，P5-P8/P10 持续生产化，N9-N12 已完成。当前主线为 N13 可信成本账与绩效重建。`2026-08-04` 已完成 OC 兼容修复生产复测：54 条六账户查询全部唯一 completed final，144 个正持仓成本完整，当日订单/成交/ETF 划转/费用通过 26 项质量检查。历史 DLQ 一次性修复也已闭环：`307000051387/2026-07-30` 的 14 条错误 Relay summary 按交割单唯一匹配修正，29 个已成交订单实际费用 55.05 元全部落账，两个缺终态订单分别恢复为 rejected/cancelled，8 条对账差异全部关闭；56 条 DLQ 为 50 acknowledged、6 ignored、0 pending。`trade_quality.v2`、贡献和成本账费用口径一致，29/29 覆盖、未终态为 0。原始 Redis/DLQ 保留，交割单修复不扩展成标准手工补数流程；每日 OC 当日数据继续作为最终来源。正式绩效仍为只读 preview，生产保持 6 个只读账户、下单账户 0。
...
1. 继续按完整交易日验收 OC 当日订单、成交、费用、资金持仓和绩效流水线；交割单只做抽查，不形成手工补数流程。`307000051387/2026-07-30` 的 29/29 费用、两个终态和 8 条对账差异已完成一次性修复；该日仍因绩效起算基线、逆回购本金处理和策略归因等独立质量标记保持 provisional，不自动发布 finalized 绩效。
