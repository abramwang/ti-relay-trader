...
- `2026-08-26`: 排查富盈13号 `314000045768` 盘后账户级阻断。15:01 的资金、29 条持仓和 201 笔订单查询均取得唯一成功终态，但 `fill.list.query` 被 OC 以 `QUERY_FAILED / 20046:市价单不能满足成交条件撤单` 结束，Relay 因缺少权威成交查询终态拒绝写入该账户 `broker_close`；其余五账户结算通过。15:04 在 OC 在线时补查再次立即返回相同失败，确认不是超时或偶发串包。业务错误实际属于 `300750` 卖单 `external-huaxin-31400004576801-12001A180003653`，订单已为 `cancelled`、成交 `1,600/2,600`，不应污染成交查询。OC 已部署 `fa43056/fd0b89b` 并提供 `RELAY_COMPATIBILITY_NOTICE_20260826.md`；Relay 无需兼容改动。15:37 首轮验收命令发出后没有 reply，OC heartbeat 已停在 15:25:34，疑似被 15:30 关停计划停止，当前状态为已修复、待 OC 临时在线验收。Relay 不绕过查询终态门禁。详细交接和验收进度见 `docs/OC_FILL_QUERY_TERMINAL_INCIDENT_20260826.md`。
