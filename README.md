...
- 当前状态: 已推进 P1/P2/P4/P5/P6 部分底座，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台骨架、PostgreSQL 首版账本 migration、真实库 migration 验证、账本 repository 骨架、Redis reply/event 到账本同步批处理、单笔下单 API、撤单 API，以及订单/成交账本查询 API；批量下单、资金/持仓查询和常驻 worker 尚未实现。
- 最近更新时间: `2026-06-13`
...
1. 实现批量下单 API：批内校验、Redis `order.batch.submit` 写入和命令归档。
2. 实现资金/持仓查询 API，从 PostgreSQL 快照或前置查询回包读取。
3. 设计模拟柜台账表 schema。
4. 增加 Python 盘后对账与账户盈亏统计任务骨架。
5. 将数据库状态接入 `/v1/status`。
6. 初始化 Python SDK 包骨架，封装下单、撤单、订单查询和成交查询。
...
- 接口测试台当前在 API 模式下可测试单笔下单、撤单、订单查询和成交查询；9092 文档门户模式不连接 Redis 和数据库，交易接口仍需要切换 `service.mode=api` 后使用。
