...
1. 基于 Meridian `bars` 补全账户绩效序列、回撤和研究侧导出视图。
2. 增加 9092 页面冒烟测试脚本。
3. 增加 `/trade` 批量下单测试视图。
4. 设计模拟柜台账表 schema。
...
- `GET /v1/meridian/market/bars` 当前是同源薄代理，不做字段映射和持久化；后续账表计算会基于该入口读取 `market_bar.v1` 数据并写入绩效序列或研究导出视图。
