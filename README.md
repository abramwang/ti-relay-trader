...
| 安全状态 | 生产新版 OC 已加载 4 户凭据 `v2`；测试机遗留生产 crontab 导致重复 OC 会话，`10:35` 每户仍有 1 个未登录残留会话；4 个启用账户只读、0 个开放交易 |
| 当前阶段 | P0-P4 完成，P5-P8/P10 持续生产化；N8-N12 完成；N13 可信成本账与绩效重建进行中 |
| 最近确认 | `2026-09-21 10:35 Asia/Shanghai` 根因确认为测试机遗留生产 OC crontab；每户 1 个健康生产会话和 1 个未登录测试机残留会话仍同时活跃，账本数量继续闭合且无重复 |
| 更新时间 | `2026-09-21` |
...
- `2026-09-21 10:35 Asia/Shanghai` 生产“收到订单但运维心跳未登录”的根因确认为测试机遗留生产 OC crontab。事故高峰时四个启用账户各有 1 个健康、2 个未登录会话；当前每户仍有 1 个测试机残留会话持续发心跳并可能抢占查询。健康会话产生的当日订单/成交逐户闭合、语义重复为 0，账本尚未受污染。Relay 未重启、未改账，生产交易继续关闭；等待测试机停止残留进程并删除 crontab，完整证据见 [OC 生产重复会话事故](/home/ti-relay-trader/docs/OC_PROD_DUPLICATE_SESSION_INCIDENT_20260921.md:1)。
...
1. 先在测试机停止四个仍活跃的未登录残留进程并删除遗留生产 OC crontab，保留生产机每户唯一健康会话；Relay 随后复验唯一会话、四类只读查询、Stream lag 和 DLQ。未经用户明确指令不得开放生产下单。
