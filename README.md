...
- 当前状态: 已推进 P1/P2/P4/P5/P6/P7 底座并进入 P8，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台表单化 UI、接口测试台模板/静态资源拆分、成熟交易软件风格 `/trade` 手动测试终端与工作区切换、文档门户同源 `/v1/*` API 挂载、PostgreSQL 账本 migration、真实库 migration 验证、`stream_checkpoints` 位点表、`job_runs` 任务运行表、账本 repository 骨架、Redis reply/event 到账本同步批处理、9092 轻量后台 Redis reply/event 同步循环、正式 worker 模式持久化位点同步、单笔下单 API、批量下单 API、撤单 API、下单幂等重放/冲突保护、资金/持仓查询与前置刷新 API、默认当日订单/成交查询、历史订单/成交/持仓查询口径、订单/成交/持仓 cursor 分页、前置/柜台拒单错误信息回写订单账本和 `/trade` 展示、订单累计成交量到成交账本的汇总补齐、账本 API 时间字段东八区格式化和零值时间省略、`POST /v1/settlements/snapshots` 日终快照落盘、`reconciliation_inputs/reconciliation_breaks` 对账输入和差异表、`GET /v1/accounts/{account_id}/performance/daily` 日终权益/PnL 输入汇总、`GET /v1/accounts/{account_id}/performance/series` close 净值绩效序列、`GET /v1/accounts/{account_id}/performance/series.csv` 绩效 CSV 导出、`order_page/fill_page` reply 合并、订单/成交事件驱动的资金持仓自动限频刷新、`GET /v1/events/stream` SSE 实时事件流、`GET /v1/status` 依赖健康/交易阶段/日流程任务状态检查、Python SDK 标准库 HTTP 客户端、订单状态/成交回调、P8 helper 方法和内网 tar.gz 安装包下载入口，以及 Meridian 主数据/快照/bars 薄代理、`/trade` 代码补全/行情刷新、股票 2 位/ETF 3 位价格精度、`/trade#performance` 绩效分析工作区、取消 `/trade` 单独盘后对账入口、订单监控/资金持仓按交易日查询及服务端分页、交易测试视图右侧持仓版面压缩、`/trade#performance` ECharts 分钟线、非交易日 bars 最近交易日回退和买卖订单/成交点位标注；已固化 `Asia/Shanghai` 东八区业务时区和交易日盘前初始化/收盘后结算流程，并新增 `python -m relay.jobs.pre_open_init`、`python -m relay.jobs.post_close_settlement` 任务骨架、`--persist` 报告落盘和收盘后 close 快照/reconciliation run 落盘；worker 已能持续消费 `reply/event/hb/dlq` 并恢复 stream 位点，心跳状态建模和 DLQ 告警仍待补充。
- 当前 9092 运行态: 使用未跟踪本地配置 `config/relay.local.yaml` 启动文档门户和同源 API，已接入测试 PostgreSQL、测试 Redis 和测试账户 `00030484`；`/v1/status` 可查看数据库、Redis、订单服务、行情代理、事件流和自动刷新状态。`000004_reconciliation_idempotency` 已应用到测试库。该文件包含凭据且不提交。
- 最近更新时间: `2026-06-14`
...
| `http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.7.tar.gz` | Python SDK 安装包 |
| `http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.7.tar.gz.sha256` | Python SDK 安装包 SHA256 |
...
- [x] `/trade#performance` 左侧新增本地 ECharts 当日分钟线；若当天不是交易日，bars 请求会通过 Meridian 交易日接口回退到最近交易日。
- [x] `/trade#performance` 分钟线支持当前账户、标的、交易日的买卖点标注：优先使用成交价/成交时间，未成交订单使用委托价/委托时间，并在订单/成交 SSE 推送后刷新标注。
...
- `/trade#performance` 分钟线图只用于手工测试与点位理解；买卖点来自本地订单/成交账本，不新增行情字段定义。成交点优先于委托点，同一订单已有成交时不会重复绘制委托价。
...
- Python SDK 当前可用 `PYTHONPATH=sdk/python`、`python -m pip install -e sdk/python` 或 `python -m pip install "http://relay-trader.quantstage.com/sdk/relay-sdk-0.1.7.tar.gz"` 安装；安装包由 `scripts/build-python-sdk.py` 生成并提交到 `public/sdk/`。
