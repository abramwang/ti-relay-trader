...
- 当前状态: 已推进 P1/P2/P4/P5/P6 部分底座，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台表单化 UI、接口测试台模板/静态资源拆分、成熟交易软件风格 `/trade` 手动测试终端与工作区切换、文档门户同源 `/v1/*` API 挂载、PostgreSQL 首版账本 migration、真实库 migration 验证、账本 repository 骨架、Redis reply/event 到账本同步批处理、9092 轻量后台 Redis reply/event 同步循环、单笔下单 API、批量下单 API、撤单 API、资金/持仓查询与前置刷新 API、订单/成交账本查询 API、订单/成交事件驱动的资金持仓自动限频刷新，以及 Meridian 主数据/快照薄代理、`/trade` 代码补全/行情刷新和股票 2 位/ETF 3 位价格精度；正式位点持久化 worker、订单/成交前置查询刷新、实时 SSE/WebSocket 推送和 Python SDK 尚未实现。
...
7. 增加正式常驻 worker，持久化 stream 位点并持续同步 `reply/event/hb/dlq`。
