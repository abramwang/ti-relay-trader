...
1. 与用户确认三类策略的归因边界、绩效正式净值、日内资产桥、逆回购、ETF 申赎、费用和外部现金流口径；确认前只建设数据质量展示，不修改计算公式。
