...
- 当前状态: 已推进 P1/P2/P4/P5/P6 部分底座，完成服务模式、配置加载、结构化日志、统一响应 envelope、API/worker 启动骨架、交易 schema、Redis 只读探测、接口测试台骨架、PostgreSQL 首版账本 migration、真实库 migration 验证、账本 repository 骨架、Redis reply/event 到账本同步批处理、单笔下单 API、批量下单 API、撤单 API、资金/持仓查询 API，以及订单/成交账本查询 API；常驻 worker、前置查询刷新和 Python SDK 尚未实现。
- 最近更新时间: `2026-06-13`
...
2. 增加前置查询刷新命令：资金、持仓、订单和成交 `cmd.query` 写入及 reply 合并。
3. 设计模拟柜台账表 schema。
...
- 接口测试台当前在 API 模式下可测试资金、持仓、单笔下单、批量下单、撤单、订单查询和成交查询；9092 文档门户模式不连接 Redis 和数据库，交易接口仍需要切换 `service.mode=api` 后使用。
- 资金/持仓查询当前读取 PostgreSQL 本地账表，不主动发前置 `cmd.query` 刷新；前置查询刷新会作为后续独立任务实现。
