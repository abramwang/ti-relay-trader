...
- 当前状态: P0-P4 已完成，P5-P8/P10 持续生产化。N9-N11 已完成，N12 剩余 API Console 断言集合、券商测试环境批量下单视图和 API/worker 进程拆分。当前主线为 N13 可信成本账与绩效重建：`performance_economic_nav.v2`、账户起算锚点、Meridian 持仓重估、移动加权成本账、数量桥阻断、成本试算/重建 API 和页面第七项质量检查已落地。首批范围为三个新账户 `307000051387/1388/1389` 与只做股票截面的债享5号 `314000046830`；`307000051387` 的 `2026-07-29` 百万元外部出金已确认落账并解除当日阻断。当前等待四账户费用参数、该户 `2026-07-30` 权威成交回填和 `2026-07-31` close 快照补齐。生产仍为 6 个只读账户、下单账户 0。
...
1. 完成 N13 四账户金标验收：通过 OC 权威查询/重放修复 `307000051387/2026-07-30` 错配成交，补齐 `2026-07-31` close 快照并配置四账户真实费率；该户 `2026-07-29` 百万元外部出金已确认落账。
...
- N13 金标验收仍有明确数据阻塞：`307000051387/2026-07-30` 有 18 个证券数量桥不平，其中 `510810.SH` 委托成交量 `76,400` 而成交表只有 `300`；在 OC 提供权威成交回填前，Relay 保持该日 blocked，不生成临时补账。`2026-07-29` 百万元出金已确认落账，不再是阻塞项。
...
- `2026-08-01`: 进入 N13 可信成本账与绩效重建：新增 `000020_performance_cost_ledger`、账户级起算锚点、逐日移动加权成本账、数量桥阻断、成本试算/重建 API 与 `relayctl performance-rebuild`。`performance_economic_nav.v2` 使用可见资金、Meridian `pre_close/close` 持仓重估和逆回购应收计算 NAV，隔离柜台 ETF 成本污染；无 v2 的旧现金快照不再绘制正式收益。生产已应用 migration 20，并为 `307000051387/1388/1389` 和债享5号 `314000046830` 保存可审计起算配置。
...
- `2026-08-01`: 复核债享5号 `314000046830` 的最早绩效边界：当前只显示 `2026-07-29..30` 是因为起算锚点被保守设在 7 月 29 日，不是页面区间问题。账本从 6 月 15 日开始，6 月 25 日起单日 v2 行情估值完整；但 7 月 2 日有历史 OC 成交错配，7 月 10 日 `cash_total` 已包含逆回购本金而 v2 又增加应收，重复约 `2,505,780.06` 元，7 月 17 日无交易却恰好减少 `3,000,000` 元资金。无需额外修正可先把起点提前至 7 月 24 日；修复逆回购现金去重、确认 300 万资金流并补实际费用后，可从 7 月 3 日重新锚定连续可信曲线。
- `2026-08-01`: 债享5号初步排查曾把 7 月 10 日空仓日初视为干净锚点；后续逐日 v2 试算发现该日 `cash_total` 已包含逆回购占款，不能直接用作未修正锚点，最终边界以上一条复核结论为准。另保留两个后续核对项：确认 7 月 27 日隔夜减少的 1 万元资金性质，并把 `512760.SH` 二级市场交易归入 ETF 截面而非 ETF 申赎 T0。
