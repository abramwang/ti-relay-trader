...
- `2026-08-26`: 排查富盈13号 `314000045768` 盘后账户级阻断。15:01 的资金、29 条持仓和 201 笔订单查询均取得唯一成功终态，但 `fill.list.query` 被 OC 以 `QUERY_FAILED / 20046:市价单不能满足成交条件撤单` 结束，Relay 因缺少权威成交查询终态拒绝写入该账户 `broker_close`；其余五账户结算通过。业务错误实际属于 `300750` 卖单 `external-huaxin-31400004576801-12001A180003653`，订单已为 `cancelled`、成交 `1,600/2,600`，不应污染成交查询。OC 部署 `fa43056/fd0b89b` 后于 15:40 生产复测仍失败；新版原始 payload 显示 `broker_callback=OnRspQryTrade`、`broker_error_id=0`、`broker_error_text=20046...` 和 `partial_data_returned=true`，说明生产错误码只留在文本或在结构体传递中被归零，未命中按数值 20046 的隔离分支。Relay 无需兼容改动且继续保持终态门禁，等待 OC 覆盖该真实回调组合后再补结算。详细证据和验收进度见 `docs/OC_FILL_QUERY_TERMINAL_INCIDENT_20260826.md`。
