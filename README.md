...
- 当前状态: P0-P4 已完成，P5-P8/P10 持续生产化，N9-N12 已完成。当前主线为 N13 可信成本账与绩效重建。债享5号已从 `2026-07-22` 空仓锚点完成连续重建；富盈13号 `2026-06-18..2026-08-03` 的 16,781 个普通成交订单已与券商交割单全部闭合。`2026-08-05` 两个 `159381.SZ` ETF T0 真实样本的 PCF、49 个实物成分划转和 IOPV + 15bp 待结算估值已经闭合。盘后流程已拆成 `post_close_capture -> post_close_settlement -> performance publish -> performance_daily`：15:01 先固化最终资金持仓为 `broker_close`，生成 close 后由服务器本机 `relayctl` 发布非阻断成本账和 provisional NAV，最后记录质量及 `published/preview_only`；公网绩效写开关保持关闭。`2026-08-20` 三个高订单量账户因单户约 3,450 条订单分片在 60 秒门限后才完成而被捕获任务阻断，已依据完整 late reply 和 15:03 资金复查完成六账户汇总捕获、结算与绩效；被替代的局部任务记录已备份后清理。生产盘后查询现优先资金持仓，等待门限提升为可配置的 180 秒；结算成功重跑会自动关闭本轮未复现的旧 open 断点，避免历史瞬时失败继续阻断复核页。生产保持 6 个只读账户、下单账户 0。
...
- 最新运行恢复: `2026-08-26` 已兼容 Meridian 当日日线 `503 archive_incomplete`，当前日线未发布时继续使用同日 Level1 `pre_close/last`。16:08 绩效恢复任务得到 3 个 `ready/published`、1 个 `not_applicable`、0 个 blocked；估值均明确标记 Level1 provisional，等待 canonical 日线水位到达后重算。
- 最近更新时间: `2026-08-26`
...
- Meridian 最新日线契约会在交易日分区尚未发布时返回 `503 archive_incomplete`；绩效模块对东八区当前交易日会保留该证据并继续使用 Level1 realtime `pre_close/last`，历史日期不降级。`2026-08-26` 现场确认 Level1 归档/质量于约 15:54 完成，而 canonical 日线父任务计划 16:30 执行；双方按“Level1 provisional、日线水位到达后重算”协调，详见 `docs/MERIDIAN_POSTCLOSE_READINESS_COORDINATION_20260826.md`。
