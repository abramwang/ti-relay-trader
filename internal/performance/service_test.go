...
func TestCalculateEconomicNAVAddsETFReceiptPostedAfterCloseSnapshot(t *testing.T) {
	store := &fakePerformanceStore{
		daily: ledger.DailyPerformance{
			AccountID:          "acct-1",
			TradeDate:          "2026-08-07",
		...
		cashByClass: map[string][]ledger.CashLedgerEntry{
			"settlement_adjustment": {{
				EntryID:     "etf-refund-after-close-20260807",
				AccountID:   "acct-1",
				TradeDate:   "2026-08-07",
				LedgerType:  "settlement",
				FlowClass:   "settlement_adjustment",
	...
	assertClose(t, result.NAV.CloseEconomicNAV, 1_000_100)
	assertClose(t, result.NAV.AccountDayPnL, 40)
	assertClose(t, result.Valuation.CloseVisibleCash, 1_000_000)
	assertClose(t, result.Valuation.PostCloseSettlementCash, 100)
	assertClose(t, result.ETFSettlement.PostCloseReceiptAmount, 100)
	for _, flag := range []string{"etf_settlement_receipt_after_close_snapshot", "post_close_cash_added_to_economic_nav"} {
		if !containsString(result.QualityFlags, flag) {
			t.Fatalf("missing %s in %#v", flag, result.QualityFlags)
		}
	}
